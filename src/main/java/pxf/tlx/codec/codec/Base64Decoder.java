...
public class Base64Decoder {
     ...
     * @return 被加密后的字符串
     */
    public static String decodeStr(CharSequence source, Charsets charset) {
    ...
    public static byte[] decode(CharSequence source) {
        return decode(ToolString.bytes(source, DEFAULT_CHARSET));
    }
    ...
    public static byte[] decode(byte[] in, int pos, int length) {
        ...
        if (octetId == octet.length) {
            ...
            return (byte[]) ToolArray.copy(octet, new byte[octetId], octetId);
        }
    }
