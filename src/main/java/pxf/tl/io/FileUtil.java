...
import pxf.tl.io.file.FileReader;
import pxf.tl.io.file.FileWriter;
import pxf.tl.io.file.*;
import pxf.tl.io.resource.ResourceUtil;
import pxf.tl.io.unit.DataSizeUtil;
import pxf.tl.util.*;
...
public class FileUtil extends PathUtil {
     ...
     */
    public static final String PATH_FILE_PRE = ToolURL.FILE_URL_PREFIX;
    /**
     * 文件路径分隔符<br>
    ...
    public static File[] ls(String path) {
        ...
        throw new IORuntimeException(ToolString.format("Path [{}] is not directory!", path));
    }
    ...
    public static List<String> listFileNames(String path) throws IORuntimeException {
        ...
        try {
            ...
            return ToolZip.listFileNames(
                    jarFile, ToolString.removePrefix(path.substring(index + 1), "/"));
        } catch (IOException e) {
            throw new IORuntimeException(ToolString.format("Can not read file path of [{}]", path), e);
        } finally {
            ToolIO.closes(jarFile);
        }
    }
    ...
    /**
     ...
     //     * @return 该文件总行数
     //     */
//    public static int getTotalLines(File file,Charsets charsets) {
//        if (false == isFile(file)) {
//            throw new IORuntimeException("Input must be a File");
//        }
//        try (final LineNumberReader lineNumberReader =
//                     new LineNumberReader(new FileReader(file,charsets))) {
//            // 设置起始为1
...
//
     ...
     * 某个文件删除失败会终止删除操作
     *
     * <p>从5.7.6开始，删除文件使用{@link Files#delete(Path)}代替 {@link File#delete()}<br>
    ...
    /**
     * 在默认临时文件目录下创建临时文件，创建后的文件名为 prefix[Randon].tmp。 默认临时文件目录由系统属性 {@code java.io.tmpdir} 指定。 在 UNIX
     * 系统上，此属性的默认值通常是 {@code "tmp"} 或 {@code "vartmp"}； 在 Microsoft Windows 系统上，它通常是 {@code
     * "C:\\WINNT\\TEMP"}。 调用 Java 虚拟机时，可以为该系统属性赋予不同的值，但不保证对该属性的编程更改对该方法使用的临时目录有任何影响。
     *
     * @return 临时文件
     * @throws IORuntimeException IO异常
     */
    ...
    /**
     * 在默认临时文件目录下创建临时文件，创建后的文件名为 prefix[Randon].suffix。 默认临时文件目录由系统属性 {@code java.io.tmpdir} 指定。 在
     * UNIX 系统上，此属性的默认值通常是 {@code "tmp"} 或 {@code "vartmp"}； 在 Microsoft Windows 系统上，它通常是 {@code
     * "C:\\WINNT\\TEMP"}。 调用 Java 虚拟机时，可以为该系统属性赋予不同的值，但不保证对该属性的编程更改对该方法使用的临时目录有任何影响。
     *
     ...
     * @throws IORuntimeException IO异常
     */
    public static File createTempFile(String prefix, String suffix, boolean isReCreat)
     ...
     * </pre>
     *
     * <p>2、isRetainExt为false时，不保留原扩展名，需要在newName中
     *
     * <pre>
    ...
    public static String getAbsolutePath(String path, Class<?> baseClass) {
        ...
        if (null != url) {
            ...
            return FileUtil.normalize(ToolURL.getDecodedPath(url));
        }

        // 如果资源不存在，则返回一个拼接的资源绝对路径
        final String classPath = JavaEnvironment.JAVA_CLASS_PATH;
        if (null == classPath) {
     ...
     * <ul>
     *   <li>以/开头的路径
     *   <li>满足类似于 c:/xxxxx，其中祖母随意，不区分大小写
     *   <li>满足类似于 d:\xxxxx，其中祖母随意，不区分大小写
     * </ul>
     *
    ...
    public static boolean isAbsolutePath(String path) {
        ...
        // 给定的路径已经是绝对路径了
        return PoolOfCharacter.SLASH == path.charAt(0) || ToolRegex.isMatch(PATTERN_PATH_ABSOLUTE, path);
    }
    ...
    public static boolean contentEquals(File file1, File file2) throws IORuntimeException {
        ...
        try {
            ...
            return ToolIO.contentEquals(input1, input2);

        } finally {
            ToolIO.closes(input1);
            ToolIO.closes(input2);
        }
    }
     ...
     * @param file1    文件1
     * @param file2    文件2
     * @param charsets 编码，null表示使用平台默认编码 两个文件内容一致返回true，否则false
     * @return 是否相同
     * @throws IORuntimeException IO异常
     */
    public static boolean contentEqualsIgnoreEOL(File file1, File file2, Charsets charsets)
            throws IORuntimeException {
        boolean file1Exists = file1.exists();
        if (file1Exists != file2.exists()) {
            return false;
        ...
        Reader input2 = null;
        try {
            input1 = getReader(file1, charsets);
            input2 = getReader(file2, charsets);
            return ToolIO.contentEqualsIgnoreEOL(input1, input2);
        } finally {
            ToolIO.closes(input1);
            ToolIO.closes(input2);
        }
    ...
    public static boolean pathEquals(File file1, File file2) {
        if (isWindows()) {
            // Windows环境
            try {
                if (ToolString.equalsIgnoreCase(file1.getCanonicalPath(), file2.getCanonicalPath())) {
                    return true;
                }
            } catch (Exception e) {
                if (ToolString.equalsIgnoreCase(file1.getAbsolutePath(), file2.getAbsolutePath())) {
                    return true;
                }
            }
        } else {
            // 类Unix环境
            try {
                if (ToolString.equals(file1.getCanonicalPath(), file2.getCanonicalPath())) {
                    return true;
                }
            } catch (Exception e) {
                if (ToolString.equals(file1.getAbsolutePath(), file2.getAbsolutePath())) {
                    return true;
                }
            }
        }
    ...
    public static int lastIndexOfSeparator(String filePath) {
        if (Whether.noEmpty(filePath)) {
            int i = filePath.length();
            char c;
            while (--i >= 0) {
                c = filePath.charAt(i);
                if (ToolChar.isFileSeparator(c)) {
                    return i;
                }
            }
        }
    ...
    public static String normalize(String path) {
        ...
        String pathToUse = ToolString.removePrefixIgnoreCase(path, ToolURL.CLASSPATH_URL_PREFIX);
        // 去除file:前缀
        pathToUse = ToolString.removePrefixIgnoreCase(pathToUse, ToolURL.FILE_URL_PREFIX);

        // 识别home目录形式，并转换为绝对路径
        if (ToolString.startWith(pathToUse, '~')) {
            pathToUse = getUserHomePath() + pathToUse.substring(1);
        }

        // 统一使用斜杠
        pathToUse = pathToUse.replaceAll("[/\\\\]+", PoolOfString.SLASH);
        // 去除开头空白符，末尾空白符合法，不去除
        pathToUse = ToolString.trimStart(pathToUse);
        // 兼容Windows下的共享目录路径（原始路径如果以\\开头，则保留这种路径）
        if (path.startsWith("\\\\")) {
            pathToUse = "\\" + pathToUse;
        }

        String prefix = ToolString.EMPTY;
        int prefixIndex = pathToUse.indexOf(PoolOfString.COLON);
        if (prefixIndex > -1) {
            // 可能Windows风格路径
            prefix = pathToUse.substring(0, prefixIndex + 1);
            if (ToolString.startWith(prefix, PoolOfCharacter.SLASH)) {
                // 去除类似于/C:这类路径开头的斜杠
                prefix = prefix.substring(1);
            }
            if (false == prefix.contains(PoolOfString.SLASH)) {
                pathToUse = pathToUse.substring(prefixIndex + 1);
            } else {
                // 如果前缀中包含/,说明非Windows风格path
                prefix = ToolString.EMPTY;
            }
        }
        if (pathToUse.startsWith(PoolOfString.SLASH)) {
            prefix += PoolOfString.SLASH;
            pathToUse = pathToUse.substring(1);
        }

        String[] pathList = ToolString.split(pathToUse, PoolOfCharacter.SLASH);
        ...
        for (int i = pathList.length - 1; i >= 0; i--) {
            element = pathList[i];
            // 只处理非.的目录，即只处理非当前目录
            if (false == PoolOfString.DOT.equals(element)) {
                if (PoolOfString.DOUBLE_DOT.equals(element)) {
                    tops++;
                } else {
                    if (tops > 0) {
                        // 有上级目录标记时按照个数依次跳过
        ...
        // issue#1703@Github
        if (tops > 0 && Whether.empty(prefix)) {
            // 只有相对路径补充开头的..，绝对路径直接忽略之
            while (tops-- > 0) {
                // 遍历完节点发现还有上级标注（即开头有一个或多个..），补充之
                // Normal path element found.
                pathElements.add(0, PoolOfString.DOUBLE_DOT);
            }
        }

        return prefix + ToolCollection.join(pathElements, PoolOfString.SLASH);
    }
    ...
    public static String subPath(String dirPath, String filePath) {
        if (Whether.noEmpty(dirPath) && Whether.noEmpty(filePath)) {

            dirPath = ToolString.removeSuffix(normalize(dirPath), "/");
            filePath = normalize(filePath);

            final String result = ToolString.removePrefixIgnoreCase(filePath, dirPath);
            return ToolString.removePrefix(result, "/");
        }
        return filePath;
    }
    ...
    public static BufferedReader getBOMReader(File file) {
        return New.bufferedReader(getBOMInputStream(file));
    }
     ...
     * @throws IORuntimeException IO异常
     */
    public static BufferedReader getReader(File file, Charsets charsets) throws IORuntimeException {
        return New.bufferedReader(getInputStream(file), charsets);
    }
     ...
     * @throws IORuntimeException IO异常
     */
    public static BufferedReader getReader(String path, Charsets charsets) throws IORuntimeException {
        return getReader(file(path), charsets);
    }
     ...
     * @throws IORuntimeException IO异常
     */
    public static String readString(File file, Charsets charsets) throws IORuntimeException {
        return FileReader.create(file, charsets).readString();
    }
     ...
     * @throws IORuntimeException IO异常
     */
    public static String readString(String path, Charsets charsets) throws IORuntimeException {
        return readString(file(path), charsets);
    }
     ...
     * @throws IORuntimeException IO异常
     */
    public static String readString(URL url, Charsets charsets) throws IORuntimeException {
        if (url == null) {
        ...
        try {
            in = url.openStream();
            return ToolIO.read(in, charsets);
        } catch (IOException e) {
            throw new IORuntimeException(e);
        } finally {
            ToolIO.closes(in);
    ...
    public static <T extends Collection<String>> T readLines(
            String path, Charsets charsets, T collection) throws IORuntimeException {
        return readLines(file(path), charsets, collection);
    }
     ...
     * @throws IORuntimeException IO异常
     */
    public static <T extends Collection<String>> T readLines(File file, Charsets charsets, T collection)
            throws IORuntimeException {
        return FileReader.create(file, charsets).readLines(collection);
    }
     ...
     * @throws IORuntimeException IO异常
     */
    public static <T extends Collection<String>> T readLines(URL url, Charsets charsets, T collection)
            throws IORuntimeException {
        InputStream in = null;
        try {
            in = url.openStream();
            return ToolIO.readLines(in, charsets, collection);
        } catch (IOException e) {
            throw new IORuntimeException(e);
        } finally {
            ToolIO.closes(in);
     ...
     * @throws IORuntimeException IO异常
     */
    public static List<String> readLines(URL url, Charsets charsets) throws IORuntimeException {
        return readLines(url, charsets, new ArrayList<>());
    }
     ...
     * @throws IORuntimeException IO异常
     */
    public static List<String> readLines(String path, Charsets charsets) throws IORuntimeException {
        return readLines(path, charsets, new ArrayList<>());
    }
     ...
     * @throws IORuntimeException IO异常
     */
    public static List<String> readLines(File file, Charsets charsets) throws IORuntimeException {
        return readLines(file, charsets, new ArrayList<>());
    }
     ...
     * @throws IORuntimeException IO异常
     */
    public static void readLines(File file, Charsets charsets, LineHandler lineHandler)
            throws IORuntimeException {
        FileReader.create(file, charsets).readLines(lineHandler);
    }
     ...
     * @throws IORuntimeException IO异常
     */
    public static void readLines(RandomAccessFile file, Charsets charsets, LineHandler lineHandler) {
        String line;
        try {
            while ((line = file.readLine()) != null) {
                lineHandler.handle(Charsets.convert(line, Charsets.ISO_8859_1, charsets));
            }
        } catch (IOException e) {
            throw new IORuntimeException(e);
        }
     ...
     * @throws IORuntimeException IO异常
     */
    public static void readLine(RandomAccessFile file, Charsets charsets, LineHandler lineHandler) {
        final String line = readLine(file, charsets);
        if (null != line) {
     ...
     * @throws IORuntimeException IO异常
     */
    public static String readLine(RandomAccessFile file, Charsets charsets) {
        String line;
        try {
            line = file.readLine();
        } catch (IOException e) {
        ...
        if (null != line) {
            return Charsets.convert(line, Charsets.ISO_8859_1, charsets);
        }
     ...
     * @throws IORuntimeException IO异常
     */
    public static <T> T load(String path, Charsets charsets, FunctionThrow<BufferedReader, T, IOException> readerHandler)
            throws IORuntimeException {
        return FileReader.create(file(path), charsets).read(readerHandler);
    }
     ...
     * @throws IORuntimeException IO异常
     */
    public static <T> T load(File file, Charsets charsets, FunctionThrow<BufferedReader, T, IOException> readerHandler)
            throws IORuntimeException {
        return FileReader.create(file, charsets).read(readerHandler);
    }
     ...
     * @throws IORuntimeException IO异常
     */
    public static BufferedWriter getWriter(String path, Charsets charsets, boolean isAppend)
            throws IORuntimeException {
        return getWriter(touch(path), charsets, isAppend);
    }
     ...
     * @throws IORuntimeException IO异常
     */
    public static BufferedWriter getWriter(File file, Charsets charsets, boolean isAppend)
            throws IORuntimeException {
        return FileWriter.create(file, charsets).getWriter(isAppend);
    }
     ...
     * @throws IORuntimeException IO异常
     */
    public static PrintWriter getPrintWriter(String path, Charsets charsets, boolean isAppend)
            throws IORuntimeException {
        return new PrintWriter(getWriter(path, charsets, isAppend));
    }
     ...
     * @throws IORuntimeException IO异常
     */
    public static PrintWriter getPrintWriter(File file, Charsets charsets, boolean isAppend)
            throws IORuntimeException {
        return new PrintWriter(getWriter(file, charsets, isAppend));
    }
     ...
     * @throws IORuntimeException IO异常
     */
    public static File writeString(String content, String path, Charsets charsets)
            throws IORuntimeException {
        return writeString(content, touch(path), charsets);
    }
     ...
     * @throws IORuntimeException IO异常
     */
    public static File writeString(String content, File file, Charsets charsets)
            throws IORuntimeException {
        return FileWriter.create(file, charsets).write(content);
    }
     ...
     * @throws IORuntimeException IO异常
     */
    public static File appendString(String content, File file, Charsets charsets)
            throws IORuntimeException {
        return FileWriter.create(file, charsets).append(content);
    }
     ...
     * @throws IORuntimeException IO异常
     */
    public static <T> File writeLines(Collection<T> list, File file, Charsets charsets)
            throws IORuntimeException {
        return writeLines(list, file, charsets, false);
    }
     ...
     * @throws IORuntimeException IO异常
     */
    public static <T> File appendLines(Collection<T> list, File file, Charsets charsets)
            throws IORuntimeException {
        return writeLines(list, file, charsets, true);
    }
     ...
     * @throws IORuntimeException IO异常
     */
    public static <T> File writeLines(Collection<T> list, File file, Charsets charsets, boolean isAppend)
            throws IORuntimeException {
        return FileWriter.create(file, charsets).writeLines(list, isAppend);
    }
    ...
    public static File writeMap(
            Map<?, ?> map, File file, Charsets charsets, String kvSeparator, boolean isAppend)
            throws IORuntimeException {
        return FileWriter.create(file, charsets).writeMap(map, kvSeparator, isAppend);
    }
     ...
     * 可读的文件大小<br>
     * 参考 <a
     * href="http://stackoverflow.com/questions/3263892/format-file-size-as-mb-gb-etc">http://stackoverflow.com/questions/3263892/format-file-size-as-mb-gb-etc</a>
     *
     * @param size Long类型大小
     * @return 大小
     ...
     * @return 被修改的文件
     */
    public static File convertLineSeparator(File file, Charsets charsets, LineSeparator lineSeparator) {
        final List<String> lines = readLines(file, charsets);
        return FileWriter.create(file, charsets).writeLines(lines, lineSeparator, false);
    }
    ...
    public static Checksum checksum(File file, Checksum checksum) throws IORuntimeException {
        ...
        try {
            return ToolIO.checksum(new FileInputStream(file), checksum);
    ...
    public static String getMimeType(String filePath) {
        ...
        if (null == contentType) {
            // 补充一些常用的mimeType
            if (ToolString.endWithIgnoreCase(filePath, ".css")) {
                contentType = "text/css";
            } else if (ToolString.endWithIgnoreCase(filePath, ".js")) {
                contentType = "application/x-javascript";
            } else if (ToolString.endWithIgnoreCase(filePath, ".rar")) {
                contentType = "application/x-rar-compressed";
            } else if (ToolString.endWithIgnoreCase(filePath, ".7z")) {
     ...
     * @param handler  行处理器
     */
    public static void tail(File file, Charsets charsets, LineHandler handler) {
        new Tailer(file, charsets, handler).start();
    }
     ...
     * @param charsets 编码
     */
    public static void tail(File file, Charsets charsets) {
        tail(file, charsets, Tailer.CONSOLE_HANDLER);
    }
    ...
    private static File buildFile(File outFile, String fileName) {
        ...
        if (false == isWindows()
                ...
                && fileName.lastIndexOf(PoolOfCharacter.SLASH, fileName.length() - 2) > 0) {
            // 在Linux下多层目录创建存在问题，/会被当成文件名的一部分，此处做处理
            // 使用/拆分路径（zip中无\），级联创建父目录
            final List<String> pathParts = ToolString.split(fileName, '/', false, true);
