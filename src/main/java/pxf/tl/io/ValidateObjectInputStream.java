...
public class ValidateObjectInputStream extends ObjectInputStream {
     ...
     */
    public ValidateObjectInputStream(InputStream inputStream, Class<?>... acceptClasses)
            throws IOException {
        super(inputStream);
    ...
    private void validateClassName(String className) throws InvalidClassException {
        // 黑名单
        if (Whether.noEmpty(this.blackClassSet)) {
            if (this.blackClassSet.contains(className)) {
                throw new InvalidClassException(
                        "Unauthorized deserialization attempt by black list", className);
            }
        }

        if (Whether.empty(this.whiteClassSet)) {
            return;
        }
