...
import pxf.tl.concurrent.lock.LockUtil;
...
public class FileAppender implements Serializable {
     ...
     * @param isNewLineMode 追加内容是否为新行
     */
    public FileAppender(File destFile, Charsets charsets, int capacity, boolean isNewLineMode) {
        this(destFile, charsets, capacity, isNewLineMode, null);
    }
     ...
     */
    public FileAppender(
            File destFile, Charsets charsets, int capacity, boolean isNewLineMode, Lock lock) {
        this.capacity = capacity;
        this.list = new ArrayList<>(capacity);
        this.isNewLineMode = isNewLineMode;
        this.writer = FileWriter.create(destFile, charsets);
        this.lock = Safe.value(lock, LockUtil::getNoLock);
    }
