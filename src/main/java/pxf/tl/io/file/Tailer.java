...
public class Tailer implements Serializable {
    ...
    public Tailer(File file, LineHandler lineHandler, int initReadLine) {
        this(file, Charsets.UTF_8, lineHandler, initReadLine, DateUnit.SECOND.getMillis());
    }
     ...
     * @param lineHandler 行处理器
     */
    public Tailer(File file, Charsets charsets, LineHandler lineHandler) {
        this(file, charsets, lineHandler, 0, DateUnit.SECOND.getMillis());
    }
     ...
     */
    public Tailer(
            File file, Charsets charset, LineHandler lineHandler, int initReadLine, long period) {
        checkFile(file);
    ...
    private void readTail() throws IOException {
        ...
        if (initReadLine > 0) {
            ...
            while (nextEnd > start) {
                ...
                if (c == PoolOfCharacter.LF || c == PoolOfCharacter.CR) {
