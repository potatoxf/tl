...
import pxf.tl.io.file.visitor.CopyVisitor;
import pxf.tl.io.file.visitor.DelVisitor;
import pxf.tl.io.file.visitor.MoveVisitor;
...
public class PathUtil {
     ...
     *
     * <ul>
     *   <li>源文件为目录，目标也为目录或不存在，则拷贝整个目录到目标目录下
     *   <li>源文件为文件，目标为目录或不存在，则拷贝文件到目标目录下
     *   <li>源文件为文件，目标也为文件，则在{@link StandardCopyOption#REPLACE_EXISTING}情况下覆盖之
     * </ul>
     *
     ...
     *
     * <ul>
     *   <li>源文件为目录，目标也为目录或不存在，则拷贝目录下所有文件和目录到目标目录下
     *   <li>源文件为文件，目标为目录或不存在，则拷贝文件到目标目录下
     * </ul>
     *
     * @param src     源文件路径，如果为目录只在目标中创建新目录
     ...
     * @throws IORuntimeException IO异常
     */
    public static Path copyContent(Path src, Path target, CopyOption... options)
            throws IORuntimeException {
        Assert.notNull(src, "Src path must be not null !");
        Assert.notNull(target, "Target path must be not null !");
     ...
     * @throws IORuntimeException IO异常
     */
    public static BufferedReader getReader(Path path, Charsets charset) throws IORuntimeException {
        return New.bufferedReader(getInputStream(path), charset);
    }
     ...
     *
     * <ul>
     *   <li>move("/usr/aaa/abc.txt", "/usr/bbb")结果为："/usr/bbb/abc.txt"
     *   <li>move("/usr/aaa", "/usr/bbb")结果为："/usr/bbb/aaa"
     * </ul>
     *
     * @param src        源文件或目录路径
     ...
     *
     * <ul>
     *   <li>moveContent("/usr/aaa/abc.txt", "/usr/bbb")结果为："/usr/bbb/abc.txt"
     *   <li>moveContent("/usr/aaa", "/usr/bbb")结果为："/usr/bbb"
     * </ul>
     *
     * @param src        源文件或目录路径
     * @param target     目标路径，如果为目录，则移动到此目录下
