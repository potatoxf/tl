...
import pxf.tl.lang.copier.SrcToDestCopier;
...
public class FileCopier extends SrcToDestCopier<File, FileCopier> {
    ...
    public File copy() throws IORuntimeException {
        ...
        if (src.isDirectory()) { // 复制目录
            ...
            final File subTarget =
                    isCopyContentIfDir ? dest : FileUtil.mkdir(FileUtil.file(dest, src.getName()));
    ...
    private void internalCopyDirContent(File src, File dest) throws IORuntimeException {
        if (null != copyFilter && false == copyFilter.test(src)) {
            // 被过滤的目录跳过
            return;
        }
        ...
        if (false == dest.exists()) {
            ...
            throw new IORuntimeException(
                    String.format(
                            "Src [%s] is a directory but dest [%s] is a file!", src.getPath(), dest.getPath()));
        }
