...
class DBFHeader {
    ...
    void read(DataInput dataInput) throws IOException {
        ...
        numberOfRecords = DBFBase.readLittleEndianInt(dataInput); /* 4-7 */

        headerLength = DBFBase.readLittleEndianShort(dataInput); /* 8-9 */
        recordLength = DBFBase.readLittleEndianShort(dataInput); /* 10-11 */

        reserv1 = DBFBase.readLittleEndianShort(dataInput); /* 12-13 */
        incompleteTransaction = dataInput.readByte(); /* 14 */
        encryptionFlag = dataInput.readByte(); /* 15 */
        freeRecordThread = DBFBase.readLittleEndianInt(dataInput); /* 16-19 */
        ...
        reserv4 = DBFBase.readLittleEndianShort(dataInput); /* 30-31 */
        ...
        fieldArray = new DBFField[dbfFields.size()];

        for (int i = 0; i < fieldArray.length; i++) {

            fieldArray[i] = dbfFields.get(i);
        }
    ...
    void write(DataOutput dataOutput) throws IOException {
        ...
        numberOfRecords = ToolMath.reserveEndian(numberOfRecords);
        dataOutput.writeInt(numberOfRecords); /* 4-7 */

        headerLength = findHeaderLength();
        dataOutput.writeShort(ToolMath.reserveEndian(headerLength)); /* 8-9 */

        recordLength = findRecordLength();
        dataOutput.writeShort(ToolMath.reserveEndian(recordLength)); /* 10-11 */

        dataOutput.writeShort(ToolMath.reserveEndian(reserv1)); /* 12-13 */
        dataOutput.writeByte(incompleteTransaction); /* 14 */
        dataOutput.writeByte(encryptionFlag); /* 15 */
        dataOutput.writeInt(ToolMath.reserveEndian(freeRecordThread)); /* 16-19 */
        dataOutput.writeInt(ToolMath.reserveEndian(reserv2)); /* 20-23 */
        dataOutput.writeInt(ToolMath.reserveEndian(reserv3)); /* 24-27 */

        dataOutput.writeByte(mdxFlag); /* 28 */
        dataOutput.writeByte(languageDriver); /* 29 */
        dataOutput.writeShort(ToolMath.reserveEndian(reserv4)); /* 30-31 */

        for (DBFField dbfField : fieldArray) {

            // System.out.println( "Length: " + fieldArray[i].getFieldLength());
            dbfField.write(dataOutput);
        }
    ...
    private short findRecordLength() {
        ...
        for (DBFField dbfField : fieldArray) {
            ...
            recordLength += dbfField.getFieldLength();
        }
