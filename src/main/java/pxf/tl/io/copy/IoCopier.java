package pxf.tl.io.copy;


import pxf.tl.api.JavaEnvironment;
import pxf.tl.io.StreamProgress;
...
public abstract class IoCopier<S, T> {
     ...
     * 构造
     *
     * @param bufferSize 缓存大小，&lt; 0 表示默认{@link JavaEnvironment#DEFAULT_BUFFER_SIZE}
     * @param count      拷贝总数，-1表示无限制
     * @param progress   进度条
     */
    public IoCopier(int bufferSize, long count, StreamProgress progress) {
        this.bufferSize = bufferSize > 0 ? bufferSize : JavaEnvironment.DEFAULT_BUFFER_SIZE;
