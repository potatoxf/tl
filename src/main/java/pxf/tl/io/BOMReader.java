...
public class BOMReader extends Reader {
    ...
    public BOMReader(InputStream in) {
        ...
        final BOMInputStream bin =
                (in instanceof BOMInputStream) ? (BOMInputStream) in : new BOMInputStream(in);
        try {
            this.reader = new InputStreamReader(bin, bin.getCharsets().get());
        } catch (Exception e) {
            throw new UnsupportedException(in.getClass().toString(), e);
        }
    }
