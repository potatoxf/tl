...
public class WatchServer extends Thread implements Closeable, Serializable {
    ...
    public void registerPath(Path path, int maxDepth) {
        final WatchEvent.Kind<?>[] kinds = Safe.value(this.events, WatchKind.ALL);

        try {
            final WatchKey key;
            if (Whether.empty(this.modifiers)) {
     ...
     */
    public void watch(WatchAction action, Predicate<WatchEvent<?>> watchFilter) {
        WatchKey wk;
        try {
