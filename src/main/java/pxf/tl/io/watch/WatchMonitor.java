...
import pxf.tl.io.watch.watchers.WatcherChain;
...
public class WatchMonitor extends WatchServer {
    ...
    public static WatchMonitor create(URL url, int maxDepth, WatchEvent.Kind<?>... events) {
        return create(New.uri(url), maxDepth, events);
    }
    ...
    public void init() throws WatchException {
        ...
        if (false == Files.exists(this.path, LinkOption.NOFOLLOW_LINKS)) {
            ...
            if (null != lastPathEle) {
                ...
                if (ToolString.contains(lastPathEleStr, PoolOfCharacter.DOT)
                        && false == ToolString.endWithIgnoreCase(lastPathEleStr, ".d")) {
    ...
    private void doTakeAndWatch(Watcher watcher) {
        super.watch(
                watcher,
                watchEvent -> null == filePath || filePath.endsWith(watchEvent.context().toString()));
    }
