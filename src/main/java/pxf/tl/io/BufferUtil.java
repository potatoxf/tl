...
public class BufferUtil {
    ...
    public static ByteBuffer copy(
            ByteBuffer src, int srcStart, ByteBuffer dest, int destStart, int length) {
     ...
     * @return 字符串
     */
    public static String readStr(ByteBuffer buffer, Charsets charsets) {
        return New.string(readBytes(buffer), charsets);
    }
    ...
    public static int lineEnd(ByteBuffer buffer, int maxLength) {
        ...
        while (buffer.hasRemaining()) {
            b = buffer.get();
            charIndex++;
            if (b == PoolOfCharacter.CR) {
                canEnd = true;
            } else if (b == PoolOfCharacter.LF) {
            ...
            if (charIndex - primitivePosition > maxLength) {
                ...
                throw new IndexOutOfBoundsException(
                        ToolString.format("Position is out of maxLength: {}", maxLength));
            }
        }
     ...
     * @return {@link ByteBuffer}
     */
    public static ByteBuffer create(CharSequence data, Charsets charsets) {
        return create(ToolString.bytes(data, charsets));
    }
    ...
    public static ByteBuffer createUtf8(CharSequence data) {
        return create(ToolString.utf8Bytes(data));
    }
