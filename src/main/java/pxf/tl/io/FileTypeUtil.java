 ...
 * 文件类型判断工具类
 *
 * <p>此工具根据文件的前几位bytes猜测文件类型，对于文本、zip判断不准确，对于视频、图片类型判断准确
 *
 * <p>需要注意的是，xlsx、docx等Office2007格式，全部识别为zip，因为新版采用了OpenXML格式，这些格式本质上是XML文件打包为zip
 *
 * @author potatoxf
 */
public class FileTypeUtil {
    ...
    public static String getType(String fileStreamHexHead) {
        for (Entry<String, String> fileTypeEntry : FILE_TYPE_MAP.entrySet()) {
            if (ToolString.startWithIgnoreCase(fileStreamHexHead, fileTypeEntry.getKey())) {
    ...
    public static String getType(InputStream in) throws IORuntimeException {
        return getType(ToolIO.readHex28Upper(in));
    }

    /**
     * 根据文件流的头部信息获得文件类型 注意此方法会读取头部28个bytes，造成此流接下来读取时缺少部分bytes<br>
     * 因此如果想服用此流，流需支持{@link InputStream#reset()}方法。
     *
     * <pre>
     *     1、无法识别类型默认按照扩展名识别
