...
public class DateModifier {
     ...
     * 可选是否归零毫秒。
     *
     * <p>在{@link ModifyType#TRUNCATE}模式下，毫秒始终要归零, 但是在{@link ModifyType#CEILING}和{@link
     * ModifyType#ROUND}模式下， 有时候由于毫秒部分必须为0（如MySQL数据库中），因此在此加上选项。
     *
     * @param calendar            {@link Calendar}
     * @param dateField           日期字段，即保留到哪个日期字段
     * @param modifyType          修改类型，包括舍去、四舍五入、进一等
     ...
     * @return 修改后的{@link Calendar}
     */
    public static Calendar modify(
            Calendar calendar, int dateField, ModifyType modifyType, boolean truncateMillisecond) {
        // AM_PM上下午特殊处理
        if (Calendar.AM_PM == dateField) {
            boolean isAM = ToolTime.isAM(calendar);
            switch (modifyType) {
                case TRUNCATE:
        ...
        for (int i = dateField + 1; i <= endField; i++) {
            if (ToolArray.contains(IGNORE_FIELDS, i)) {
    ...
    private static void modifyField(Calendar calendar, int field, ModifyType modifyType) {
        ...
        switch (modifyType) {
            case TRUNCATE:
                calendar.set(field, ToolTime.getBeginValue(calendar, field));
                break;
            case CEILING:
                calendar.set(field, ToolTime.getEndValue(calendar, field));
                break;
            case ROUND:
                int min = ToolTime.getBeginValue(calendar, field);
                int max = ToolTime.getEndValue(calendar, field);
                int href;
