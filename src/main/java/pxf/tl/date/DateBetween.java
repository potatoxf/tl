...
public class DateBetween implements Serializable {
    ...
    public long betweenMonth(boolean isReset) {
        final Calendar beginCal = ToolTime.calendar(begin);
        final Calendar endCal = ToolTime.calendar(end);
    ...
    public long betweenYear(boolean isReset) {
        ...
        if (false == isReset) {
            // 考虑闰年的2月情况
            if (Calendar.FEBRUARY == beginCal.get(Calendar.MONTH)
                    && Calendar.FEBRUARY == endCal.get(Calendar.MONTH)) {
