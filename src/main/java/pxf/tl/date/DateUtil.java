...
import pxf.tl.date.format.DatePrinter;
import pxf.tl.date.format.FastDateFormat;
...
public class DateUtil {
     ...
     */
    public static int[] yearAndQuarter(Date date) {
        return ToolTime.yearAndQuarter(ToolTime.calendar(date));
    }
     ...
     */
    public static LinkedHashSet<int[]> yearAndQuarter(Date startDate, Date endDate) {
        if (startDate == null || endDate == null) {
            return new LinkedHashSet<>(0);
        }
        return ToolTime.yearAndQuarter(startDate.getTime(), endDate.getTime());
    }
    // ------------------------------------ Format start
    // ----------------------------------------------
    ...
    public static String format(Date date, String format) {
        if (null == date || Whether.blank(format)) {
            return null;
        }
     ...
     *
     * @param date   被格式化的日期
     * @param format {@link DatePrinter} 或 {@link FastDateFormat} {@link
    ...
    public static String format(Date date, DateTimeFormatter format) {
        ...
        return ToolTime.format(date.toInstant(), format);
    }
    ...
    public static String formatChineseDate(Date date, boolean isUppercase, boolean withTime) {
        ...
        if (false == isUppercase) {
            return (withTime ? DatePattern.CHINESE_DATE_TIME_FORMAT : DatePattern.CHINESE_DATE_FORMAT)
                    .format(date);
        }

        return ToolTime.formatChineseDate(ToolTime.calendar(date), withTime);
    }
    // ------------------------------------ Format end ----------------------------------------------
    ...
    public static DateTime parse(String str, String... parsePatterns) throws DateException {
        return new DateTime(ToolTime.parseByPatterns(str, parsePatterns));
    }
    ...
    public static DateTime parseTimeToday(CharSequence timeString) {
        timeString = ToolString.format("{} {}", today(), timeString);
        if (1 == ToolString.count(timeString, ':')) {
            // 时间格式为 HH:mm
     ...
     *   <li>yyyy-MM-dd'T'HH:mm:ss+0800
     *   <li>yyyy-MM-dd'T'HH:mm:ss+08:00
     * </ol>
     *
    ...
    public static DateTime parseUTC(String utcString) {
        ...
        if (ToolString.contains(utcString, 'Z')) {
        ...
        } else if (ToolString.contains(utcString, '+')) {
            // 去除类似2019-06-01T19:45:43 +08:00加号前的空格
            utcString = utcString.replace(" +", "+");
            final String zoneOffset = ToolString.subAfter(utcString, '+', true);
            if (Whether.blank(zoneOffset)) {
                throw new DateException("Invalid format: [{}]", utcString);
            }
            if (false == ToolString.contains(zoneOffset, ':')) {
                // +0800转换为+08:00
                final String pre = ToolString.subBefore(utcString, '+', true);
                utcString = pre + "+" + zoneOffset.substring(0, 2) + ":" + "00";
            }

            if (ToolString.contains(utcString, PoolOfCharacter.DOT)) {
            ...
            } else if (ToolString.contains(utcString, PoolOfCharacter.DOT)) {
     ...
     *
     * <ol>
     *   <li>EEE MMM dd HH:mm:ss z yyyy（例如：Wed Aug 01 00:00:00 CST 2012）
     * </ol>
     *
     ...
     *   <li>yyyy-MM-dd HH:mm:ss.SSSSSS
     *   <li>yyyyMMddHHmmss
     *   <li>yyyyMMddHHmmssSSS
     *   <li>yyyyMMdd
     *   <li>EEE, dd MMM yyyy HH:mm:ss z
    ...
    public static DateTime parse(CharSequence dateCharSequence) {
        if (Whether.blank(dateCharSequence)) {
            return null;
        }
        String dateStr = dateCharSequence.toString();
        // 去掉两边空格并去掉中文日期中的“日”和“秒”，以规范长度
        dateStr = dateStr.trim().replace('日', '秒');
        int length = dateStr.length();

        if (ToolNumber.isNumber(dateStr)) {
            // 纯数字形式
        ...
        if (ToolRegex.isMatch(DatePattern.REGEX_NORM, dateStr)) {
            final int colonCount = ToolString.count(dateStr, PoolOfCharacter.COLON);
            switch (colonCount) {
                case 0:
                    // yyyy-MM-dd
                ...
                case 2:
                    final int indexOfDot = ToolString.indexOf(dateStr, PoolOfCharacter.DOT);
                    if (indexOfDot > 0) {
                        ...
                        if (length1 - indexOfDot > 4) {
                            ...
                            dateStr = ToolString.subPre(dateStr, indexOfDot + 4);
                        }
    ...
    public static DateTime truncate(Date date, DateField dateField) {
        return new DateTime(ToolTime.truncate(ToolTime.calendar(date), dateField));
    }
    ...
    public static DateTime round(Date date, DateField dateField) {
        return new DateTime(ToolTime.round(ToolTime.calendar(date), dateField));
    }
    ...
    public static DateTime ceiling(Date date, DateField dateField) {
        return new DateTime(ToolTime.ceiling(ToolTime.calendar(date), dateField));
    }
    ...
    public static DateTime ceiling(Date date, DateField dateField, boolean truncateMillisecond) {
        return new DateTime(ToolTime.ceiling(ToolTime.calendar(date), dateField, truncateMillisecond));
    }
    ...
    public static DateTime beginOfSecond(Date date) {
        return new DateTime(ToolTime.beginOfSecond(ToolTime.calendar(date)));
    }
    ...
    public static DateTime endOfSecond(Date date) {
        return new DateTime(ToolTime.endOfSecond(ToolTime.calendar(date)));
    }
    ...
    public static DateTime beginOfHour(Date date) {
        return new DateTime(ToolTime.beginOfHour(ToolTime.calendar(date)));
    }
    ...
    public static DateTime endOfHour(Date date) {
        return new DateTime(ToolTime.endOfHour(ToolTime.calendar(date)));
    }
    ...
    public static DateTime beginOfMinute(Date date) {
        return new DateTime(ToolTime.beginOfMinute(ToolTime.calendar(date)));
    }
    ...
    public static DateTime endOfMinute(Date date) {
        return new DateTime(ToolTime.endOfMinute(ToolTime.calendar(date)));
    }
    ...
    public static DateTime beginOfDay(Date date) {
        return new DateTime(ToolTime.beginOfDay(ToolTime.calendar(date)));
    }
    ...
    public static DateTime endOfDay(Date date) {
        return new DateTime(ToolTime.endOfDay(ToolTime.calendar(date)));
    }
    ...
    public static DateTime beginOfWeek(Date date) {
        return new DateTime(ToolTime.beginOfWeek(ToolTime.calendar(date)));
    }
    ...
    public static DateTime beginOfWeek(Date date, boolean isMondayAsFirstDay) {
        return new DateTime(ToolTime.beginOfWeek(ToolTime.calendar(date), isMondayAsFirstDay));
    }
    ...
    public static DateTime endOfWeek(Date date) {
        return new DateTime(ToolTime.endOfWeek(ToolTime.calendar(date)));
    }
    ...
    public static DateTime endOfWeek(Date date, boolean isSundayAsLastDay) {
        return new DateTime(ToolTime.endOfWeek(ToolTime.calendar(date), isSundayAsLastDay));
    }
    ...
    public static DateTime beginOfMonth(Date date) {
        return new DateTime(ToolTime.beginOfMonth(ToolTime.calendar(date)));
    }
    ...
    public static DateTime endOfMonth(Date date) {
        return new DateTime(ToolTime.endOfMonth(ToolTime.calendar(date)));
    }
    ...
    public static DateTime beginOfQuarter(Date date) {
        return new DateTime(ToolTime.beginOfQuarter(ToolTime.calendar(date)));
    }
    ...
    public static DateTime endOfQuarter(Date date) {
        return new DateTime(ToolTime.endOfQuarter(ToolTime.calendar(date)));
    }
    ...
    public static DateTime beginOfYear(Date date) {
        return new DateTime(ToolTime.beginOfYear(ToolTime.calendar(date)));
    }
    ...
    public static DateTime endOfYear(Date date) {
        return new DateTime(ToolTime.endOfYear(ToolTime.calendar(date)));
    }
    // --------------------------------------------------- Offset for now
    ...
    public static boolean isSameDay(final Date date1, final Date date2) {
        ...
        return ToolTime.isSameDay(ToolTime.calendar(date1), ToolTime.calendar(date2));
    }
    ...
    public static boolean isSameWeek(final Date date1, final Date date2, boolean isMon) {
        ...
        return ToolTime.isSameWeek(ToolTime.calendar(date1), ToolTime.calendar(date2), isMon);
    }
    ...
    public static boolean isSameMonth(final Date date1, final Date date2) {
        ...
        return ToolTime.isSameMonth(ToolTime.calendar(date1), ToolTime.calendar(date2));
    }
    ...
    public static int age(Date birthday, Date dateToCompare) {
        ...
        return ToolTime.computeAge(birthday.getTime(), dateToCompare.getTime());
    }
    ...
    public static boolean isExpired(
            Date startDate, DateField dateField, int timeLength, Date endDate) {
     ...
     * 判定在指定检查时间是否过期。
     *
     * <p>以商品为例，startDate即生产日期，endDate即保质期的截止日期，checkDate表示在何时检查是否过期（一般为当前时间）<br>
    ...
    public static int timeToSecond(String timeStr) {
        if (Whether.empty(timeStr)) {
            return 0;
        }

        final List<String> hms = ToolString.splitTrim(timeStr, PoolOfString.COLON, 3);
    ...
    public static List<DateTime> rangeContains(DateRange start, DateRange end) {
        List<DateTime> startDateTimes = New.list(false, (Iterable<DateTime>) start);
        List<DateTime> endDateTimes = New.list(false, (Iterable<DateTime>) end);
    ...
    public static List<DateTime> rangeNotContains(DateRange start, DateRange end) {
        List<DateTime> startDateTimes = New.list(false, (Iterable<DateTime>) start);
        List<DateTime> endDateTimes = New.list(false, (Iterable<DateTime>) end);
        return endDateTimes.stream()
                .filter(item -> !startDateTimes.contains(item))
    ...
    public static <T> List<T> rangeFunc(
            Date start, Date end, final DateField unit, Function<Date, T> func) {
    ...
    public static void rangeConsume(
            Date start, Date end, final DateField unit, Consumer<Date> consumer) {
        if (start == null || end == null || start.after(end)) {
            return;
        }
    ...
    public static List<DateTime> rangeToList(Date start, Date end, DateField unit) {
        return New.list(false, (Iterable<DateTime>) range(start, end, unit));
    }
    ...
    public static List<DateTime> rangeToList(Date start, Date end, final DateField unit, int step) {
        return New.list(false, (Iterable<DateTime>) new DateRange(start, end, unit, step));
    }
    ...
    public static int compare(Date date1, Date date2) {
        return ToolCompare.compare(date1, date2);
    }
    ...
    public static Instant toInstant(TemporalAccessor temporalAccessor) {
        return ToolTime.toInstant(temporalAccessor);
    }
    ...
    public static DateTime convertTimeZone(Date date, ZoneId zoneId) {
        return new DateTime(date, ToolTime.toTimeZone(zoneId));
    }
    ...
    public static boolean isOverlap(
            Date realStartTime, Date realEndTime, Date startTime, Date endTime) {
    ...
    private static String normalize(CharSequence dateStr) {
        if (Whether.blank(dateStr)) {
            return ToolString.str(dateStr);
        }

        // 日期时间分开处理
        final List<String> dateAndTime = ToolString.splitTrim(dateStr, ' ');
        ...
        datePart = ToolString.removeSuffix(datePart, "日");
        ...
        if (size == 2) {
            ...
            timePart = ToolString.removeSuffix(timePart, ":");
            // 将ISO8601中的逗号替换为.
