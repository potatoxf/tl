package pxf.tl.swing.clipboard;
...
public enum ClipboardMonitor implements ClipboardOwner, Runnable, Closeable {
    ...
    ClipboardMonitor(int tryCount, long delay) {
        this(tryCount, delay, ClipboardHelper.getClipboard());
    }
    ...
    public void lostOwnership(Clipboard clipboard, Transferable contents) {
        ...
        for (ClipboardListener listener : listenerSet) {
            try {
                transferable = listener.onChange(clipboard, Safe.value(transferable, newContents));
            } catch (Throwable e) {
                // 忽略事件处理异常，保证所有监听正常执行
            }
        }
        ...
        if (isRunning) {
            ...
            clipboard.setContents(
                    Safe.value(transferable, Safe.value(newContents, contents)), this);
        }
    }
