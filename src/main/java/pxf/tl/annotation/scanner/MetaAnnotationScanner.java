...
/**
 * 扫描注解类上存在的注解，支持处理枚举实例或枚举类型 需要注意，当待解析是枚举类时，有可能与{@link TypeAnnotationScanner}冲突
 *
 * @author potatoxf
...
public class MetaAnnotationScanner implements AnnotationScanner {
    ...
    public boolean support(AnnotatedElement annotatedEle) {
        return (annotatedEle instanceof Class
                && ToolClass.isAssignable(Annotation.class, (Class<?>) annotatedEle));
    }
    ...
    public List<Annotation> getAnnotations(AnnotatedElement annotatedEle) {
        ...
        scan(
                ...
                annotatedEle,
                annotation -> ToolObject.notEqual(annotation, annotatedEle));
        return annotations;
    }
    ...
    public void scan(
        ...
        filter = Safe.value(filter, t -> true);
        Set<Class<? extends Annotation>> accessed = new HashSet<>();
        final Deque<List<Class<? extends Annotation>>> deque = new LinkedList<>();
        deque.add(New.list(false, (Class<? extends Annotation>) annotatedEle));
        int distance = 0;
        do {
            ...
            for (final Class<? extends Annotation> type : annotationTypes) {
                final List<Annotation> metaAnnotations =
                ...
                accessed.add(type);
                List<Class<? extends Annotation>> next =
                        metaAnnotations.stream()
                                .map(Annotation::annotationType)
                                .filter(t -> !accessed.contains(t))
                                .collect(Collectors.toList());
                if (Whether.noEmpty(next)) {
                    deque.addLast(next);
                }
            }
            distance++;
        } while (includeSupperMetaAnnotation && Whether.noEmpty(deque));
    }
}
