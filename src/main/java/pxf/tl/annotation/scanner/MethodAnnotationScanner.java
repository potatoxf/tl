...
public class MethodAnnotationScanner extends AbstractTypeAnnotationScanner<MethodAnnotationScanner>
    ...
    public MethodAnnotationScanner(boolean scanSameSignatureMethod) {
        this(scanSameSignatureMethod, targetClass -> true, New.set(true));
    }
     ...
     */
    public MethodAnnotationScanner(
            boolean scanSameSignatureMethod, Predicate<Class<?>> filter, Set<Class<?>> excludeTypes) {
    ...
    protected Annotation[] getAnnotationsFromTargetClass(
        ...
        return Stream.of(ToolBytecode.getMethods(targetClass))
    ...
    private boolean hasSameSignature(Method sourceMethod, Method superMethod) {
        if (false == ToolString.equals(sourceMethod.getName(), superMethod.getName())) {
            return false;
        }
        ...
        if (!ToolArray.containsAll(sourceParameterTypes, targetParameterTypes)) {
            return false;
        }
        return ToolClass.isAssignable(superMethod.getReturnType(), sourceMethod.getReturnType());
    }
}
