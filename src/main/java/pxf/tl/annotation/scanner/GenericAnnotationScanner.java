...
import pxf.tl.collection.map.multi.ListValueMap;
 ...
 *
 * <ul>
 *   <li>当元素为{@link Method}时，此处层级结构指声明方法的类的层级结构， 扫描器将从层级结构中寻找与该方法签名相同的方法，并对其进行扫描；
 *   <li>当元素为{@link Class}时，此处层级结构即指类本身与其父类、父接口共同构成的层级结构， 扫描器将扫描层级结构中类、接口声明的注解；
 *   <li>当元素不为{@link Method}或{@link Class}时，则其层级结构仅有其本身一层；
 * </ul>
 * <p>
...
public class GenericAnnotationScanner implements AnnotationScanner {
    ...
    public GenericAnnotationScanner(
        ...
        this.metaScanner =
                enableScanMetaAnnotation ? new MetaAnnotationScanner() : new EmptyAnnotationScanner();
        this.typeScanner =
    ...
    private void scanElements(
            ...
            Predicate<Annotation> filter) {
        // 扫描类上注解
        final ListValueMap<Integer, Annotation> classAnnotations =
                new ListValueMap<>(new LinkedHashMap<>());
        scanner.scan(
                (index, annotation) -> {
