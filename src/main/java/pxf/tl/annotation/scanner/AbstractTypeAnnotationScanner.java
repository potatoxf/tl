...
public abstract class AbstractTypeAnnotationScanner<T extends AbstractTypeAnnotationScanner<T>>
    ...
    protected AbstractTypeAnnotationScanner(
    ...
    public T addExcludeTypes(Class<?>... excludeTypes) {
        ToolCollection.addAll(this.excludeTypes, excludeTypes);
        return typedThis;
    }
    ...
    public T addConverters(UnaryOperator<Class<?>> converter) {
        ...
        if (!this.hasConverters) {
            this.hasConverters = Whether.noEmpty(this.converters);
        }
        return typedThis;
    }
    ...
    public void scan(
        ...
        final Deque<List<Class<?>>> classDeque =
                new LinkedList<>();
        classDeque.add(New.list(false, sourceClass));
        final Set<Class<?>> accessedTypes = new LinkedHashSet<>();
        int index = 0;
        while (Whether.noEmpty(classDeque)) {
            ...
            for (Class<?> targetClass : currClassQueue) {
                ...
                // 处理层级索引和注解
                final Annotation[] targetAnnotations =
                        getAnnotationsFromTargetClass(annotatedEle, index, targetClass);
            ...
            if (Whether.noEmpty(nextClassQueue)) {
    ...
    protected abstract Annotation[] getAnnotationsFromTargetClass(
            AnnotatedElement source, int index, Class<?> targetClass);
    ...
    protected boolean isNotNeedProcess(Set<Class<?>> accessedTypes, Class<?> targetClass) {
        return Whether.nvl(targetClass)
    ...
    protected void scanInterfaceIfNecessary(List<Class<?>> nextClasses, Class<?> targetClass) {
        if (includeInterfaces) {
            ...
            if (Whether.noEmpty(interfaces)) {
                ToolCollection.addAll(nextClasses, interfaces);
            }
        }
    }
    ...
    protected void scanSuperClassIfNecessary(List<Class<?>> nextClassQueue, Class<?> targetClass) {
        if (includeSuperClass) {
            ...
            if (!ToolObject.equals(superClass, Object.class) && Whether.nvl(superClass)) {
