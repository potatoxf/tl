...
/**
 * 用于处理注解对象中带有{@link Link}注解，且{@link Link#type()}为{@link RelationType#MIRROR_FOR}的属性。<br>
 * 当该处理器执行完毕后，原始合成注解中被{@link Link}注解的属性与{@link Link}注解指向的目标注解的属性， 都将会被被包装并替换为{@link
...
public class MirrorLinkAnnotationPostProcessor extends AbstractLinkAnnotationPostProcessor {
    ...
    private static final RelationType[] PROCESSED_RELATION_TYPES =
            new RelationType[]{RelationType.MIRROR_FOR};
    ...
    /**
     * 将存在镜像关系的合成注解属性分别包装为{@link MirroredAnnotationAttribute}对象， 并使用包装后{@link
     * MirroredAnnotationAttribute}替换在它们对应合成注解实例中的{@link AnnotationAttribute}
     *
    ...
    protected void processLinkedAttribute(
        ...
        final AnnotationAttribute mirroredOriginalAttribute =
                new MirroredAnnotationAttribute(originalAttribute, linkedAttribute);
        originalAnnotation.setAttribute(
                originalAttribute.getAttributeName(), mirroredOriginalAttribute);
        final AnnotationAttribute mirroredTargetAttribute =
                new MirroredAnnotationAttribute(linkedAttribute, originalAttribute);
    ...
    private void checkMirrorRelation(
            Link annotation, AnnotationAttribute original, AnnotationAttribute mirror) {
        // 镜像属性必须存在
        ...
        Assert.isTrue(
                Whether.noNvl(mirrorAttributeAnnotation)
