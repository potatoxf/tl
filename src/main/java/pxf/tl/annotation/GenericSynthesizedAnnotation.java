...
public class GenericSynthesizedAnnotation<R, T extends Annotation>
    ...
    protected Map<String, AnnotationAttribute> loadAttributeMethods() {
        return Stream.of(ToolBytecode.getMethods(annotation.annotationType()))
                .filter(AnnotationUtil::isAttributeMethod)
                .collect(
                        Collectors.toMap(
                                Method::getName, method -> new CacheableAnnotationAttribute(annotation, method)));
    }
    ...
    public boolean hasAttribute(String attributeName, Class<?> returnType) {
        return Optional.ofNullable(attributeMethodCaches.get(attributeName))
                .filter(method -> ToolClass.isAssignable(returnType, method.getAttributeType()))
                .isPresent();
    }
    ...
    public Object getAttributeValue(String attributeName, Class<?> attributeType) {
        return Optional.ofNullable(attributeMethodCaches.get(attributeName))
                .filter(method -> ToolClass.isAssignable(attributeType, method.getAttributeType()))
                .map(AnnotationAttribute::getValue)
                .get();
    }
}
