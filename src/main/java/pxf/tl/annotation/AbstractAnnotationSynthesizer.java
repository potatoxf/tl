...
public abstract class AbstractAnnotationSynthesizer<T> implements AnnotationSynthesizer {
     ...
     * @return 最终所需的合成注解
     */
    protected abstract <A extends Annotation> A synthesize(
            Class<A> annotationType, SynthesizedAnnotation annotation);
    ...
    public <A extends Annotation> A synthesize(Class<A> annotationType) {
        return (A)
                synthesizedProxyAnnotations.computeIfAbsent(
                        annotationType,
                        type -> {
                            final SynthesizedAnnotation synthesizedAnnotation =
                                    synthesizedAnnotationMap.get(annotationType);
                            return Whether.nvl(synthesizedAnnotation)
                                    ? null
                                    : synthesize(annotationType, synthesizedAnnotation);
                        });
    }
}
