...
/**
 * {@link SynthesizedAggregateAnnotation}的基本实现，表示基于多个注解对象， 或多个根注解对象与他们的多层元注解对象的聚合得到的注解。
 *
 * <p>假设现有注解A，若指定的{@link #annotationScanner}支持扫描注解A的元注解，
 * 且A上存在元注解B，B上存在元注解C，则对注解A进行解析，将得到包含根注解A，以及其元注解B、C在内的合成元注解聚合{@link
 * GenericSynthesizedAggregateAnnotation}。 从{@link
 * AnnotatedElement}的角度来说，得到的合成注解是一个同时承载有ABC三个注解对象的被注解元素， 因此通过调用{@link
 * AnnotatedElement}的相关方法将返回对应符合语义的注解对象。
 *
 * <p>在扫描指定根注解及其元注解时，若在不同的层级出现了类型相同的注解实例， 将会根据实例化时指定的{@link SynthesizedAnnotationSelector}选择最优的注解，
 ...
 *
 * <p>当扫描的注解对象经过{@link SynthesizedAnnotationSelector}处理后， 将会被转为{@link
 * MetaAnnotation}，并使用在实例化时指定的{@link AliasAnnotationPostProcessor} 进行后置处理。<br>
 * 默认情况下，将注册以下后置处理器以对{@link Alias}与{@link Link}和其扩展注解提供支持：
 *
 * <ul>
 *   <li>{@link AliasAnnotationPostProcessor}；
 *   <li>{@link MirrorLinkAnnotationPostProcessor}；
 *   <li>{@link AliasLinkAnnotationPostProcessor}；
 * </ul>
 * <p>
 * 若用户需要自行扩展，则需要保证上述三个处理器被正确注入当前实例。
 *
 * <p>{@link GenericSynthesizedAggregateAnnotation}支持通过{@link #getAttributeValue(String, Class)}，
 * 或通过{@link #synthesize(Class)}获得注解代理对象后获取指定类型的注解属性值， 返回的属性值将根据合成注解中对应原始注解属性上的{@link Alias}与{@link
 * Link}注解而有所变化。 通过当前实例获取属性值时，将经过{@link SynthesizedAnnotationAttributeProcessor}的处理。<br>
...
public class GenericSynthesizedAggregateAnnotation
    ...
    /**
     * 基于指定根注解，为其层级结构中的全部注解构造一个合成注解。 若扫描器支持对注解的层级结构进行扫描，则若层级结构中出现了相同的注解对象时，
    ...
    public GenericSynthesizedAggregateAnnotation(
            List<Annotation> source, AnnotationScanner annotationScanner) {
        this(
                source,
    ...
    GenericSynthesizedAggregateAnnotation(
        ...
        this.root = Safe.value(root, this);
    ...
    protected Map<Class<? extends Annotation>, SynthesizedAnnotation> loadAnnotations() {
        ...
        for (int i = 0; i < source.size(); i++) {
            ...
            Assert.isFalse(
                    AnnotationUtil.isSynthesizedAnnotation(sourceAnnotation),
            ...
            annotationScanner.scan(
                    (index, annotation) -> {
                        ...
                        SynthesizedAnnotation newAnnotation =
                                new MetaAnnotation(sourceAnnotation, annotation, index + 1, annotationMap.size());
                        if (Whether.nvl(oldAnnotation)) {
                            annotationMap.put(annotation.annotationType(), newAnnotation);
                        } else {
                            annotationMap.put(
                                    annotation.annotationType(),
                                    annotationSelector.choose(oldAnnotation, newAnnotation));
                        }
                    },
                    sourceAnnotation.annotationType(),
                    null);
    ...
    public Object getAttributeValue(String attributeName, Class<?> attributeType) {
        return attributeProcessor.getAttributeValue(
                attributeName, attributeType, synthesizedAnnotationMap.values());
    }
     ...
     * @param annotationType 注解类型
     * @return 合成注解对象
     * @see SynthesizedAnnotationProxy#create(Class, AnnotationAttributeValueProvider,
     * SynthesizedAnnotation)
     */
    @Override
    public <T extends Annotation> T synthesize(
    ...
    public static class MetaAnnotation extends GenericSynthesizedAnnotation<Annotation, Annotation> {
         ...
         * @param horizontalDistance 距离根对象的垂直距离
         */
        protected MetaAnnotation(
                Annotation root, Annotation annotation, int verticalDistance, int horizontalDistance) {
