...
import pxf.tl.annotation.scanner.AnnotationScanner;
import pxf.tl.annotation.scanner.MetaAnnotationScanner;
import pxf.tl.annotation.scanner.MethodAnnotationScanner;
import pxf.tl.annotation.scanner.TypeAnnotationScanner;
...
public class AnnotationUtil {
    ...
    public static boolean isJdkMetaAnnotation(Class<? extends Annotation> annotationType) {
        return PoolOfObject.META_ANNOTATIONS.get().contains(annotationType);
    }
     ...
     * 获取指定注解
     *
     * @param annotationEle   {@link
     *                        AnnotatedElement}，可以是Class、Method、Field、Constructor、ReflectPermission
    ...
    public static <T> T[] getCombinationAnnotations(
            AnnotatedElement annotationEle, Class<T> annotationType) {
    ...
    public static <T> T[] getAnnotations(
            AnnotatedElement annotationEle, boolean isToCombination, Class<T> annotationType) {
        ...
        final T[] result = New.array(annotationType, annotations.length);
     ...
     */
    public static Annotation[] getAnnotations(
            AnnotatedElement annotationEle, boolean isToCombination, Predicate<Annotation> predicate) {
        ...
        return ToolArray.filter(result, predicate::test);
    }
    ...
    public static <A extends Annotation> A getAnnotation(
            AnnotatedElement annotationEle, Class<A> annotationType) {
    ...
    public static boolean hasAnnotation(
            AnnotatedElement annotationEle, Class<? extends Annotation> annotationType) {
     ...
     */
    public static <T> T getAnnotationValue(
            AnnotatedElement annotationEle,
        ...
        final Method[] methods = ToolBytecode.getMethods(annotation, propertyName, false);
        if (methods.length == 0) {
            return null;
        }
        return ToolBytecode.invokeSilent(annotation, methods[0]);
    }
    ...
    public static Map<String, Object> getAnnotationValueMap(
        ...
        final Method[] methods =
                ToolBytecode.getMethods(
                        annotationType,
                        t -> {
                            if (Whether.empty(t.getParameterTypes())) {
                                // 只读取无参方法
        ...
        for (Method method : methods) {
            result.put(method.getName(), ToolBytecode.invokeSilent(annotation, method));
        }
        return result;
    }
    ...
    /**
     * 扫描注解类，以及注解类的{@link Class}层级结构中的注解，将返回除了{@link PoolOfObject#META_ANNOTATIONS}中指定的JDK默认注解外， 按元注解对象与{@code
     * annotationType}的距离和{@link Class#getAnnotations()}顺序排序的注解对象集合
     *
     * <p>比如：<br>
    ...
    /**
     * 扫描类以及类的{@link Class}层级结构中的注解，将返回除了{@link PoolOfObject#META_ANNOTATIONS}中指定的JDK默认元注解外, 全部类/接口的{@link
     * Class#getAnnotations()}方法返回的注解对象。<br>
     * 层级结构将按广度优先递归，遵循规则如下：
     *
     * <ul>
     *   <li>同一层级中，优先处理父类，然后再处理父接口；
     *   <li>同一个接口在不同层级出现，优先选择层级距离{@code targetClass}更近的接口；
     *   <li>同一个接口在相同层级出现，优先选择其子类/子接口被先解析的那个；
     * </ul>
     * <p>
    ...
    /**
     * 扫描方法，以及该方法所在类的{@link Class}层级结构中的具有相同方法签名的方法， 将返回除了{@link PoolOfObject#META_ANNOTATIONS}中指定的JDK默认元注解外,
    ...
    public static void setValue(Annotation annotation, String annotationField, Object value) {
        final Map memberValues =
                (Map) ToolBytecode.getFieldValueAsSafeSilent(Proxy.getInvocationHandler(annotation), "memberValues");
    ...
    public static <T extends Annotation> T getSynthesizedAnnotation(
            Class<T> annotationType, Annotation... annotations) {
        // TODO 缓存合成注解信息，避免重复解析
        return Optional.ofNullable(annotations)
                .filter(Whether::noEmpty)
     ...
     *
     * <ul>
     *   <li>若元素是类，则递归解析全部父类和全部父接口上的注解;
     *   <li>若元素是方法、属性或注解，则只解析其直接声明的注解;
     * </ul>
     *
     * <p>注解合成规则如下： 若{@code AnnotatedEle}按顺序从上到下声明了A，B，C三个注解，且三注解存在元注解如下：
     *
     * <pre>
     *    A -&gt; M3
     *    B -&gt; M1 -&gt; M2 -&gt; M3
     ...
     */
    public static <T extends Annotation> T getSynthesizedAnnotation(
            AnnotatedElement annotatedEle, Class<T> annotationType) {
        T target = annotatedEle.getAnnotation(annotationType);
        if (Whether.noNvl(target)) {
            return target;
        }
     ...
     * @return 聚合注解
     */
    public static SynthesizedAggregateAnnotation aggregatingFromAnnotation(
            Annotation... annotations) {
        return new GenericSynthesizedAggregateAnnotation(
                Arrays.asList(annotations), AnnotationScanner.NOTHING);
    }
     ...
     * @return 聚合注解
     */
    public static SynthesizedAggregateAnnotation aggregatingFromAnnotationWithMeta(
            Annotation... annotations) {
        return new GenericSynthesizedAggregateAnnotation(
                Arrays.asList(annotations), AnnotationScanner.DIRECTLY_AND_META_ANNOTATION);
    }
