 ...
 *
 * <p>合成注解一般被用于处理类层级结果中具有直接或间接关联的注解对象， 当实例被创建时，会获取到这些注解对象，并使用{@link
 * SynthesizedAnnotationSelector}对类型相同的注解进行过滤， 并最终得到类型不重复的有效注解对象。这些有效注解将被包装为{@link
 * SynthesizedAnnotation}， 然后最终用于“合成”一个{@link SynthesizedAggregateAnnotation}。<br>
 * {@link SynthesizedAnnotationSelector}是合成注解生命周期中的第一个钩子， 自定义选择器以拦截原始注解被扫描的过程。
 *
 * <p>当合成注解完成对待合成注解的扫描，并完成了必要属性的加载后， 将会按顺序依次调用{@link SynthesizedAnnotationPostProcessor}，
 * 注解后置处理器允许用于对完成注解的待合成注解进行二次调整， 该钩子一般用于根据{@link Link}注解对属性进行调整。<br>
 * {@link SynthesizedAnnotationPostProcessor}是合成注解生命周期中的第二个钩子， 自定义后置处理器以拦截原始在转为待合成注解后的初始化过程。
 *
 * <p>合成注解允许通过{@link #synthesize(Class)}合成一个指定的注解对象， 该方法返回的注解对象可能是原始的注解对象，也有可能通过动态代理的方式生成，
...
public interface SynthesizedAggregateAnnotation
    ...
    /**
     * 距离{@link #getRoot()}返回值的垂直距离， 默认聚合注解即为根对象，因此返回0
     *
    ...
    /**
     * 距离{@link #getRoot()}返回值的水平距离， 默认聚合注解即为根对象，因此返回0
     *
     * @return 距离{@link #getRoot()}返回值的水平距离，
     */
    @Override
