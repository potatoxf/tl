...
/**
 * 被合成注解后置处理器，用于在{@link SynthesizedAggregateAnnotation}加载完所有待合成注解后， 再对加载好的{@link
 * SynthesizedAnnotation}进行后置处理。<br>
...
public interface SynthesizedAnnotationPostProcessor
     ...
     */
    MirrorLinkAnnotationPostProcessor MIRROR_LINK_ANNOTATION_POST_PROCESSOR =
            new MirrorLinkAnnotationPostProcessor();

    /**
     * 属性上带有{@link Link}，且与其他注解的属性存在别名关系的注解对象的后置处理器
     */
    AliasLinkAnnotationPostProcessor ALIAS_LINK_ANNOTATION_POST_PROCESSOR =
    ...
    default int compareTo(SynthesizedAnnotationPostProcessor o) {
        return ToolCompare.compare(
                this, o, Comparator.comparing(SynthesizedAnnotationPostProcessor::order));
    }
