...
public class SynthesizedAnnotationProxy implements InvocationHandler {
    ...
    SynthesizedAnnotationProxy(
        ...
        Assert.notNull(
                annotationAttributeValueProvider, "annotationAttributeValueProvider must not null");
    ...
    public static <T extends Annotation> T create(
        ...
        final SynthesizedAnnotationProxy proxyHandler =
                new SynthesizedAnnotationProxy(annotationAttributeValueProvider, annotation);
        if (Whether.nvl(annotation)) {
            return null;
        }
        return (T)
    ...
    public static boolean isProxyAnnotation(Class<?> annotationType) {
        return ToolClass.isAssignable(SyntheticProxyAnnotation.class, annotationType);
    }
    ...
    public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
        return Optional.ofNullable(methods.get(method.getName()))
                ...
                .orElseGet(() -> ToolBytecode.invokeSilent(this, method, args));
    }
    ...
    void loadMethods() {
        ...
        methods.put(
                "hasAttribute",
                (method, args) -> annotation.hasAttribute((String) args[0], (Class<?>) args[1]));
        ...
        methods.put(
                "getAttributeValue", (method, args) -> annotation.getAttributeValue((String) args[0]));
        methods.put("annotationType", (method, args) -> annotation.annotationType());
        for (final Method declaredMethod :
                ToolBytecode.getMethods(annotation.getAnnotation().annotationType())) {
    ...
    private String proxyToString() {
        final String attributes =
                Stream.of(ToolBytecode.getMethods(annotation.getAnnotation().annotationType()))
                        .filter(AnnotationUtil::isAttributeMethod)
                        .map(
                                method ->
                                        CharSequenceUtil.format("{}={}", method.getName(), proxyAttributeValue(method)))
    ...
    private Object proxyAttributeValue(Method attributeMethod) {
        return annotationAttributeValueProvider.getAttributeValue(
                attributeMethod.getName(), attributeMethod.getReturnType());
    }
