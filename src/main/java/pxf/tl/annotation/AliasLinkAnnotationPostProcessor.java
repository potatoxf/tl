...
/**
 * 用于处理注解对象中带有{@link Link}注解，且{@link Link#type()}为 {@link RelationType#ALIAS_FOR}或{@link
 * RelationType#FORCE_ALIAS_FOR}的属性。<br>
 * 当该处理器执行完毕后，{@link Link}注解指向的目标注解的属性将会被包装并替换为 {@link AliasedAnnotationAttribute}或{@link
...
public class AliasLinkAnnotationPostProcessor extends AbstractLinkAnnotationPostProcessor {
    ...
    private static final RelationType[] PROCESSED_RELATION_TYPES =
            new RelationType[]{RelationType.ALIAS_FOR, RelationType.FORCE_ALIAS_FOR};
    ...
    /**
     * 该处理器只处理{@link Link#type()}类型为{@link RelationType#ALIAS_FOR}和{@link
     * RelationType#FORCE_ALIAS_FOR}的注解属性
     *
    ...
    /**
     * 获取{@link Link}指向的目标注解属性，并根据{@link Link#type()}的类型是 {@link RelationType#ALIAS_FOR}或{@link
     * RelationType#FORCE_ALIAS_FOR} 将目标注解属性包装为{@link AliasedAnnotationAttribute}或{@link
     * ForceAliasedAnnotationAttribute}， 然后用包装后注解属性在对应的合成注解中替换原始的目标注解属性
     *
    ...
    protected void processLinkedAttribute(
        ...
        if (RelationType.ALIAS_FOR.equals(annotation.type())) {
            wrappingLinkedAttribute(
                    synthesizer, originalAttribute, linkedAttribute, AliasedAnnotationAttribute::new);
            return;
        }
        // 处理forceAliasFor类型的关系
        wrappingLinkedAttribute(
                synthesizer, originalAttribute, linkedAttribute, ForceAliasedAnnotationAttribute::new);
    }
    ...
    private void wrappingLinkedAttribute(
        ...
        // 是包装属性
        final AbstractWrappedAnnotationAttribute wrapper =
                (AbstractWrappedAnnotationAttribute) aliasAttribute;
        wrapper
    ...
    private void processAttribute(
        ...
        Optional.ofNullable(target.getAnnotationType())
                ...
                .ifPresent(
                        t ->
                                t.replaceAttribute(
                                        target.getAttributeName(), old -> wrapping.apply(old, originalAttribute)));
    }
    ...
    private void checkAliasRelation(
            Link annotation, AnnotationAttribute originalAttribute, AnnotationAttribute linkedAttribute) {
    ...
    private void checkCircularDependency(AnnotationAttribute original, AnnotationAttribute alias) {
        ...
        Link annotation =
                getLinkAnnotation(alias, RelationType.ALIAS_FOR, RelationType.FORCE_ALIAS_FOR);
        if (Whether.nvl(annotation)) {
            return;
        }
        final Class<?> aliasAnnotationType =
                getLinkedAnnotationType(annotation, alias.getAnnotationType());
        if (ToolObject.notEqual(aliasAnnotationType, original.getAnnotationType())) {
            return;
        }
        Assert.notEquals(
