 ...
 *
 * <p>该对象可通过{@link #getVerticalDistance()}与{@link #getHorizontalDistance()} 描述其在以参照物为基点的坐标坐标系中的位置。
 * <br>
 * 在需要对该接口的实现类进行按优先级排序时，距离{@link #getRoot()}对象越近，则该实现类的优先级越高。 默认提供了{@link
 * #DEFAULT_HIERARCHICAL_COMPARATOR}用于实现该比较规则。<br>
 * 一般情况下，{@link #getRoot()}返回值相同的对象之间的比较才有意义。
 *
 * <p>此外，还提供了{@link Selector}接口用于根据一定的规则从两个{@link Hierarchical}实现类中选择并返回一个最合适的对象， 默认提供了四个实现类：
 *
 * <ul>
 *   <li>{@link Selector#NEAREST_AND_OLDEST_PRIORITY}: 返回距离根对象更近的对象，当距离一样时优先返回旧对象；
 *   <li>{@link Selector#NEAREST_AND_NEWEST_PRIORITY}: 返回距离根对象更近的对象，当距离一样时优先返回新对象；
 *   <li>{@link Selector#FARTHEST_AND_OLDEST_PRIORITY}: 返回距离根对象更远的对象，当距离一样时优先返回旧对象；
 *   <li>{@link Selector#FARTHEST_AND_NEWEST_PRIORITY}: 返回距离根对象更远的对象，当距离一样时优先返回新对象；
 * </ul>
 *
 * @author potatoxf
 */
public interface Hierarchical extends Comparable<Hierarchical> {
     ...
     */
    Comparator<Hierarchical> DEFAULT_HIERARCHICAL_COMPARATOR =
    ...
    /**
     * 参照物，即坐标为{@code (0, 0)}的对象。 当对象本身即为参照物时，该方法应当返回其本身
     *
     * @return 参照物
     */
    Object getRoot();
    ...
    /**
     * 获取该对象与参照物的水平距离。 默认情况下，该距离即为当前对象在与参照物{@link #getVerticalDistance()}相同的情况下条， 该对象被扫描到的顺序。
     *
    ...
    interface Selector {
        ...
        class NearestAndOldestPrioritySelector implements Selector {
            ...
            public <T extends Hierarchical> T choose(T oldAnnotation, T newAnnotation) {
                return newAnnotation.getVerticalDistance() < oldAnnotation.getVerticalDistance()
                        ? newAnnotation
                        : oldAnnotation;
            }
        }
