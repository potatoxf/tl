...
/**
 * {@link SynthesizedAnnotationPostProcessor}的基本实现， 用于处理注解中带有{@link Link}注解的属性。
 *
 * @author potatoxf
...
public abstract class AbstractLinkAnnotationPostProcessor
    ...
    /**
     * 若一个注解属性上存在{@link Link}注解，注解的{@link Link#type()}返回值在{@link #processTypes()}中存在， 且此{@link
     * Link}指定的注解对象在当前的{@link SynthesizedAggregateAnnotation}中存在，
     * 则从聚合器中获取类型对应的合成注解对象，与该对象中的指定属性，然后将全部关联数据交给 {@link #processLinkedAttribute}处理。
     *
    ...
    public void process(
        ...
        final Map<String, AnnotationAttribute> attributeMap =
                new HashMap<>(synthesizedAnnotation.getAttributes());
        attributeMap.forEach(
                (originalAttributeName, originalAttribute) -> {
                    // 获取注解
                    ...
                    final SynthesizedAnnotation linkedAnnotation =
                            getLinkedAnnotation(link, synthesizer, synthesizedAnnotation.annotationType());
                    if (Whether.nvl(linkedAnnotation)) {
                        return;
                    }
                    final AnnotationAttribute linkedAttribute =
                            linkedAnnotation.getAttributes().get(link.attribute());
                    // 处理
    ...
    protected Link getLinkAnnotation(AnnotationAttribute attribute, RelationType... relationTypes) {
        return Optional.ofNullable(attribute)
                ...
                .filter(a -> ToolArray.contains(relationTypes, a.type()))
                .get();
    }
    ...
    /**
     * 若{@link Link#annotation()}获取的类型{@code Annotation#getClass()}，则返回{@code defaultType}， 否则返回{@link
     * Link#annotation()}指定的类型
     *
     * @param annotation  {@link Link}注解
    ...
    protected Class<?> getLinkedAnnotationType(Link annotation, Class<?> defaultType) {
        return ToolObject.equals(annotation.annotation(), Annotation.class)
                ? defaultType
    ...
    protected void checkLinkedSelf(AnnotationAttribute original, AnnotationAttribute linked) {
        boolean linkSelf =
                (original == linked) || ToolObject.equals(original.getAttribute(), linked.getAttribute());
    ...
    protected void checkLinkedAttributeNotNull(
            AnnotationAttribute original, AnnotationAttribute linkedAttribute, Link annotation) {
        Assert.notNull(
                linkedAttribute,
