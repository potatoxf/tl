 ...
 * <br>
 * 除了{@link #getValue()}以外，其他方法的返回值应当尽可能与{@link #getOriginal()} 返回的{@link
 * AnnotationAttribute}对象的方法返回值一致。
 *
 * <p>当包装类被包装了多层后，则规则生效优先级按包装的先后顺序倒序排序， 比如a、b互为镜像，此时a、b两属性应当都被{@link MirroredAnnotationAttribute}包装，
 * 若再指定c为a的别名字段，则c、a、b都要在原基础上再次包装一层{@link AliasedAnnotationAttribute}。<br>
 * 此时a、b同时被包装了两层，则执行时，优先执行{@link AliasedAnnotationAttribute}的逻辑， 当该规则不生效时，比如c只有默认值，此时上一次的{@link
 * MirroredAnnotationAttribute}的逻辑才会生效。
 *
 * <p>被包装的{@link AnnotationAttribute}实际结构为一颗二叉树， 当包装类再次被包装时，实际上等于又添加了一个新的根节点， 此时需要同时更新树的全部关联叶子节点。
 *
 * @author potatoxf
...
public interface WrappedAnnotationAttribute extends AnnotationAttribute {
    ...
    /**
     * 该注解属性的值是否等于默认值 <br>
     * 默认仅当{@link #getOriginal()}与{@link #getLinked()}返回的注解属性 都为默认值时，才返回{@code true}
     *
