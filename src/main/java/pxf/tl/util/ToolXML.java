...
import pxf.tl.collection.map.BiMap;
...
public final class ToolXML {
     ...
     * @return {@code Element}
     */
    public static Element createRootElement(Document document, String tag, String... attributes) {
        Element rootElement = document.createElement(tag);
        document.appendChild(rootElement);
        int length = attributes.length / 2 * 2;
        for (int i = 0; i < length; i += 2) {
            rootElement.setAttribute(attributes[i], attributes[i + 1]);
        }
        return rootElement;
    }
     ...
     * @param document 文档
     * @return 输出字符串
     * @throws TransformerException 如果发生异常
     */
    public static String outputString(Document document) throws TransformerException {
    ...
    public static void output(Document document, OutputStream outputStream)
        ...
        output(document, New.outputStreamWriter(outputStream, Charsets.UTF_8));
    }
     ...
     * @throws TransformerException 如果发生异常
     */
    public static void output(Document document, Writer writer) throws TransformerException {
        output(new DOMSource(document), new StreamResult(writer));
    }
     ...
     * @throws TransformerException 如果发生异常
     */
    private static void output(DOMSource source, StreamResult result) throws TransformerException {
    ...
    public static Document readXML(String pathOrContent) {
        if (ToolString.startWith(pathOrContent, '<')) {
    ...
    public static String toStr(Node doc, boolean isPretty) {
        return toStr(doc, Charsets.UTF_8, isPretty);
    }
     ...
     * @return XML字符串
     */
    public static String toStr(Node doc, Charsets charsets, boolean isPretty) {
        return toStr(doc, charsets, isPretty, false);
    }
     ...
     * @return XML字符串
     */
    public static String toStr(Document doc, Charsets charsets, boolean isPretty) {
        return toStr((Node) doc, charsets, isPretty);
    }
    ...
    public static String toStr(
            Node doc, Charsets charsets, boolean isPretty, boolean omitXmlDeclaration) {
        final StringWriter writer = New.stringWriter();
        try {
            write(doc, writer, charsets, isPretty ? INDENT_DEFAULT : 0, omitXmlDeclaration);
        } catch (Exception e) {
     ...
     * @param charsets 自定义XML文件的编码，如果为{@code null} 读取XML文档中的编码，否则默认UTF-8
     */
    public static void toFile(Document doc, String path, Charsets charsets) {
        if (Whether.blank(charsets)) {
            charsets = Charsets.parseCharsets(doc.getXmlEncoding());
        }
        if (Whether.blank(charsets)) {
        ...
        try {
            writer = FileUtil.getWriter(path, charsets, false);
            write(doc, writer, charsets, INDENT_DEFAULT);
        } finally {
     ...
     */
    public static void write(Node node, Writer writer, Charsets charsets, int indent) {
        transform(new DOMSource(node), new StreamResult(writer), charsets, indent);
    }
    ...
    public static void write(
            Node node, Writer writer, Charsets charsets, int indent, boolean omitXmlDeclaration) {
        transform(new DOMSource(node), new StreamResult(writer), charsets, indent, omitXmlDeclaration);
    }
     ...
     * @param indent   格式化输出中缩进量，小于1表示不格式化输出
     */
    public static void write(Node node, OutputStream out, Charsets charsets, int indent) {
        transform(new DOMSource(node), new StreamResult(out), charsets, indent);
    }
     ...
     */
    public static void write(
            Node node, OutputStream out, Charsets charsets, int indent, boolean omitXmlDeclaration) {
        transform(new DOMSource(node), new StreamResult(out), charsets, indent, omitXmlDeclaration);
    }
     ...
     * @param indent   格式化输出中缩进量，小于1表示不格式化输出
     */
    public static void transform(Source source, Result result, Charsets charsets, int indent) {
        transform(source, result, charsets, indent, false);
    }
    ...
    public static void transform(
            Source source, Result result, Charsets charsets, int indent, boolean omitXmlDeclaration) {
        ...
        try {
            ...
            if (Whether.noBlank(charsets)) {
                xformer.setOutputProperty(OutputKeys.ENCODING, charsets.get().name());
            }
     ...
     *
     * <p>默认使用"com.sun.org.apache.xerces.internal.jaxp.DocumentBuilderFactoryImpl"<br>
    ...
    public static DocumentBuilderFactory createDocumentBuilderFactory() {
        ...
        if (Whether.noEmpty(defaultDocumentBuilderFactory)) {
    ...
    public static Document createXml(String rootElementName, String namespace) {
        ...
        doc.appendChild(
                null == namespace
                        ? doc.createElement(rootElementName)
    ...
    public static String cleanComment(String xmlContent) {
        ...
        return xmlContent.replaceAll(COMMENT_REGEX, ToolString.EMPTY);
    }
    ...
    public static List<Element> getElements(Element element, String tagName) {
        final NodeList nodeList =
                Whether.blank(tagName) ? element.getChildNodes() : element.getElementsByTagName(tagName);
    ...
    public static Object getByXPath(
            String expression, Object source, QName returnType, NamespaceContext nsContext) {
    ...
    public static String escape(String string) {
        return ToolEscape.escapeHtml4(string);
    }
     ...
     * @param string 被替换的字符串
     * @return 替换后的字符串
     * @see ToolEscape#unescape(String)
     */
    public static String unescape(String string) {
        return ToolEscape.unescapeHtml4(string);
    }
            ...
            if (null != newValue) {
                if (null != value) {
                    if (value instanceof List) {
                        ...
                        result.put(childEle.getNodeName(), New.list(false, value, newValue));
                    }
                } else {
    ...
    public static String mapToXmlStr(Map<?, ?> data, boolean omitXmlDeclaration) {
        return toStr(mapToXml(data, "xml"), Charsets.UTF_8, false, omitXmlDeclaration);
    }
     ...
     * @return XML格式的字符串
     */
    public static String mapToXmlStr(
            Map<?, ?> data, String rootName, String namespace, boolean omitXmlDeclaration) {
        return toStr(mapToXml(data, rootName, namespace), Charsets.UTF_8, false, omitXmlDeclaration);
    }
    ...
    public static String mapToXmlStr(
        ...
        return toStr(
                mapToXml(data, rootName, namespace), Charsets.UTF_8, isPretty, omitXmlDeclaration);
    }
    ...
    public static String mapToXmlStr(
            ...
            boolean isPretty,
            boolean omitXmlDeclaration) {
        return toStr(mapToXml(data, rootName, namespace), charsets, isPretty, omitXmlDeclaration);
    }
    ...
    public static Document beanToXml(Object bean, String namespace, boolean ignoreNull) {
        ...
        return mapToXml(
                BeanUtil.beanToMap(bean, false, ignoreNull), bean.getClass().getSimpleName(), namespace);
    }
    ...
    private static Node appendText(Document doc, Node node, CharSequence text) {
        return node.appendChild(doc.createTextNode(ToolString.str(text)));
    }
    ...
    public static class UniversalNamespaceCache implements NamespaceContext {
        ...
        /**
         * This constructor parses the document and stores all namespaces it can find. If toplevelOnly
         * is true, only namespaces in the root are used.
         *
            ...
            if (false == attributesOnly) {
                ...
                if (null != childNodes) {
                    ...
                    for (int i = 0; i < childLength; i++) {
                        ...
                        if (item.getNodeType() == Node.ELEMENT_NODE) examineNode(item, false);
                    }
                }
            }
        }
