 ...
 * @author potatoxf
 */
public final class ToolEnum {

    /**
     * 找到枚举值
     *
     * @param enumClass 枚举类，不为null
     * @param value     值，如果为null
     * @param <E>       枚举类型
     * @return 如果没有找到返回默认值，否则反置值
     */
    public static <E extends Enum<E>> E findEnum(Class<E> enumClass, Integer value, E defaultValue) {
        if (enumClass == null) {
     ...
     * @return 如果没有找到返回默认值，否则反置值
     */
    public static <E extends Enum<E>> E findEnum(Class<E> enumClass, String value, E defaultValue) {
        if (enumClass == null) {
            throw new IllegalArgumentException();
        }
        if (value == null) {
        ...
        if (Whether.integerNumber(value)) {
            int i = Integer.parseInt(value);
            for (E enumConstant : enumClass.getEnumConstants()) {
                if (enumConstant.ordinal() == i) {
                    return enumConstant;
                }
            }
        } else {
    ...
    public static <E extends Enum<E>> E fromString(Class<E> enumClass, String value, E defaultValue) {
        return Safe.value(fromStringQuietly(enumClass, value), defaultValue);
    }
    ...
    public static <E extends Enum<E>> E fromStringQuietly(Class<E> enumClass, String value) {
        if (null == enumClass || Whether.blank(value)) {
            return null;
        }
    ...
    public static <E extends Enum<E>> E likeValueOf(Class<E> enumClass, Object value) {
        ...
        final Field[] fields = ToolBytecode.getFields(enumClass);
        ...
        for (Field field : fields) {
            ...
            for (Enum<?> enumObj : enums) {
                if (ToolObject.equal(value, ToolBytecode.getFieldValueAsSafeSilent(enumObj, field))) {
                    return (E) enumObj;
                }
            }
        }
    ...
    public static List<Object> getFieldValues(Class<? extends Enum<?>> clazz, String fieldName) {
        ...
        for (Enum<?> e : enums) {
            list.add(ToolBytecode.getFieldValueAsSafeSilent(e, fieldName));
        }
        return list;
    }
    ...
    public static List<String> getFieldNames(Class<? extends Enum<?>> clazz) {
        ...
        final Field[] fields = ToolBytecode.getNoStaticFields(clazz);
        String name;
        for (Field field : fields) {
            name = field.getName();
     ...
     */
    public static Map<String, Object> getNameFieldMap(
            Class<? extends Enum<?>> clazz, String fieldName) {
        final Enum<?>[] enums = clazz.getEnumConstants();
        if (null == enums) {
            return null;
        }
        final Map<String, Object> map = New.map(true, enums.length);
        for (Enum<?> e : enums) {
            map.put(e.name(), ToolBytecode.getFieldValueAsSafeSilent(e, fieldName));
        }
        return map;
    }
    ...
    public static <E extends Enum<E>> boolean contains(final Class<E> enumClass, String val) {
        return ToolEnum.getEnumMap(enumClass).containsKey(val);
    }
    ...
    public static boolean equalsIgnoreCase(final Enum<?> e, String val) {
        return ToolString.equalsIgnoreCase(toString(e), val);
    }
    ...
    public static boolean equals(final Enum<?> e, String val) {
        return ToolString.equals(toString(e), val);
    }
}
