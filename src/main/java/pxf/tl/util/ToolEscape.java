...
import pxf.tl.text.escape.Html4Escape;
import pxf.tl.text.escape.Html4Unescape;
import pxf.tl.text.escape.XmlEscape;
import pxf.tl.text.escape.XmlUnescape;
 ...
 * 所有的空格符、标点符号、特殊字符以及其他非ASCII字符都将被转化成%xx格式的字符编码(xx等于该字符在字符集表里面的编码的16进制数字)。 TODO
 * 6.x迁移到core.text.escape包下
 *
 * @author potatoxf
 */
public final class ToolEscape {
    ...
    private static final Predicate<Character> JS_ESCAPE_PREDICATE =
            c ->
                    !(Character.isDigit(c)
                            || Character.isLowerCase(c)
                            || Character.isUpperCase(c)
                            || ToolString.contains(NOT_ESCAPE_CHARS, c));
