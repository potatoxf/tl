...
public class BitSetBloomFilter implements BloomFilter {
    ...
    public static int hash(String str, int k) {
        switch (k) {
            case 0:
                return ToolHash.rsHash(str);
            case 1:
                return ToolHash.jsHash(str);
            case 2:
                return ToolHash.elfHash(str);
            case 3:
                return ToolHash.bkdrHash(str);
            case 4:
                return ToolHash.apHash(str);
            case 5:
                return ToolHash.djbHash(str);
            case 6:
                return ToolHash.sdbmHash(str);
            case 7:
                return ToolHash.pjwHash(str);
            default:
                return 0;
        }
    }
     ...
     */
    public void init(String path, Charsets charsets) throws IOException {
        BufferedReader reader = FileUtil.getReader(path, charsets);
        try {
            String line;
            while (true) {
    ...
    public double getFalsePositiveProbability() {
        // (1 - e^(-k * n / m)) ^ k
        return Math.pow(
                (1 - Math.exp(-hashFunctionNumber * (double) addedElements / bitSetSize)),
                hashFunctionNumber);
    }
}
