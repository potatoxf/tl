...
public class UploadFile {
        ...
        if (data != null) {
            ...
            if (false == this.tempFile.exists()) {
                throw new NoSuchFileException(
                        "Temp file: [" + this.tempFile.getAbsolutePath() + "] not exist!");
            }
    ...
    public byte[] getFileContent() throws IOException {
        ...
        if (tempFile != null) {
            return FileUtil.readBytes(tempFile.toPath());
        }
        return null;
    }
    ...
    protected boolean processStream(MultipartRequestInputStream input) throws IOException {
        ...
        tempFile =
                FileUtil.createTempFile(
                        TMP_FILE_PREFIX, TMP_FILE_SUFFIX, FileUtil.touch(setting.tmpUploadPath), false);
    ...
    private void assertValid() throws IOException {
        if (false == isUploaded()) {
            throw new IOException(ToolString.format("File [{}] upload fail", getFileName()));
        }
    }
    // ---------------------------------------------------------------------------- Private method end
}
