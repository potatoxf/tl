...
import pxf.tl.collection.map.TableMap;
...
public class UrlQuery {
     ...
     * @return UrlQuery
     */
    public static UrlQuery of(String queryStr, Charsets charsets) {
        return of(queryStr, charsets, true);
    }
     ...
     * @return UrlQuery
     */
    public static UrlQuery of(String queryStr, Charsets charsets, boolean autoRemovePath) {
        return of(queryStr, charsets, autoRemovePath, false);
    }
    ...
    public static UrlQuery of(
            String queryStr, Charsets charsets, boolean autoRemovePath, boolean isFormUrlEncoded) {
        return new UrlQuery(isFormUrlEncoded).parse(queryStr, charsets, autoRemovePath);
    }
    ...
    private static String toStr(Object value) {
        ...
        if (value instanceof Iterable) {
            result = ToolCollection.join((Iterable<?>) value, ",");
        } else if (value instanceof Iterator) {
            result = ToolCollection.join((Iterator<?>) value, ",");
        } else {
     ...
     * @param charsets decode编码，null表示不做decode
     * @return this
     */
    public UrlQuery parse(String queryStr, Charsets charsets) {
        return parse(queryStr, charsets, true);
    }
     ...
     * @return this
     */
    public UrlQuery parse(String queryStr, Charsets charsets, boolean autoRemovePath) {
        if (Whether.blank(queryStr)) {
            return this;
        }
        ...
        if (autoRemovePath) {
            ...
            if (pathEndPos > -1) {
                queryStr = ToolString.subSuf(queryStr, pathEndPos + 1);
                if (Whether.blank(queryStr)) {
                    return this;
                }
            }
        ...
        return doParse(queryStr, charsets);
    }
     ...
     *
     * <ul>
     *   <li>如果key为{@code null}，则这个键值对忽略
     *   <li>如果value为{@code null}，只保留key，如key1对应value为{@code null}生成类似于{@code key1&key2=v2}形式
     * </ul>
     *
     * @param charsets encode编码，null表示不做encode编码
     * @return URL查询字符串
     */
    ...
    public String build(Charsets charsets, boolean encodePercent) {
        if (isFormUrlEncoded) {
            return build(PercentCoder.ALL, PercentCoder.ALL, charsets, encodePercent);
        }

        return build(RFC3986.QUERY_PARAM_NAME, RFC3986.QUERY_PARAM_VALUE, charsets, encodePercent);
    }
     ...
     * @return URL查询字符串
     */
    public String build(PercentCoder keyCoder, PercentCoder valueCoder, Charsets charsets) {
        return build(keyCoder, valueCoder, charsets, true);
    }
     ...
     */
    public String build(
            PercentCoder keyCoder, PercentCoder valueCoder, Charsets charsets, boolean encodePercent) {
        if (Whether.empty(this.query)) {
            return ToolString.EMPTY;
        }
        keyCoder = keyCoder.orNew("%").setCharsets(charsets);
        ...
        for (Map.Entry<CharSequence, CharSequence> entry : this.query) {
            ...
            if (null != name) {
                ...
                sb.append(keyCoder.encodeString(name.toString()));
                value = entry.getValue();
                if (null != value) {
                    sb.append("=").append(valueCoder.encodeString(value.toString()));
                }
            }
        }
        return sb.toString();
     ...
     * @return this
     */
    private UrlQuery doParse(String queryStr, Charsets charsets) {
        ...
        for (i = 0; i < len; i++) {
            ...
            switch (c) {
                case '=': // 键和值的分界符
                    ...
                    break;
                case '&': // 键值对之间的分界符
                    addParam(name, queryStr.substring(pos, i), charsets);
                    name = null;
     ...
     * @param charsets 编码
     */
    private void addParam(String key, String value, Charsets charsets) {
        if (null != key) {
            final String actualKey = URLDecoder.decode(key, charsets, isFormUrlEncoded);
            this.query.put(
                    actualKey, Safe.value(URLDecoder.decode(value, charsets, isFormUrlEncoded)));
        } else if (null != value) {
            // name为空，value作为name，value赋值null
            this.query.put(URLDecoder.decode(value, charsets, isFormUrlEncoded), null);
        }
    }
}
