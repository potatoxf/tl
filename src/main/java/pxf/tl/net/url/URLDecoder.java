...
public class URLDecoder implements Serializable {
     ...
     * @return 解码后的字符串
     */
    public static String decodeForPath(String str, Charsets charsets) {
        return decode(str, charsets, false);
    }
     ...
     * @return 解码后的字符串
     */
    public static String decode(String str, Charsets charsets) {
        return decode(str, charsets, true);
    }
     ...
     * @param charsets      编码，{@code null}表示不做编码
     * @return 解码后的字符串
     */
    public static String decode(String str, Charsets charsets, boolean isPlusToSpace) {
        if (null == charsets) {
            return str;
        }
        return New.string(decode(New.bytes(str, charsets), isPlusToSpace), charsets);
    }
    ...
    public static byte[] decode(byte[] bytes, boolean isPlusToSpace) {
        ...
        for (int i = 0; i < bytes.length; i++) {
            b = bytes[i];
            if (b == '+') {
                buffer.write(isPlusToSpace ? PoolOfCharacter.SPACE : b);
            } else if (b == ESCAPE_CHAR) {
                if (i + 1 < bytes.length) {
                    final int u = ToolChar.digit16(bytes[i + 1]);
                    if (u >= 0 && i + 2 < bytes.length) {
                        final int l = ToolChar.digit16(bytes[i + 2]);
                        if (l >= 0) {
