 ...
 *
 * <ul>
 *   <li>协议（protocol），默认TLS
 *   <li>{@link KeyManager}，默认空
 *   <li>{@link TrustManager}，默认{@link DefaultTrustManager}，即信任全部
 *   <li>{@link SecureRandom}
 * </ul>
 *
 * <p>构建后可获得{@link SSLContext}，通过调用{@link SSLContext#getSocketFactory()}获取{@link
...
public class SSLContextBuilder implements SSLProtocols, Builder<SSLContext> {
    ...
    public SSLContextBuilder setTrustManagers(TrustManager... trustManagers) {
        if (Whether.noEmpty(trustManagers)) {
    ...
    public SSLContextBuilder setKeyManagers(KeyManager... keyManagers) {
        if (Whether.noEmpty(keyManagers)) {
