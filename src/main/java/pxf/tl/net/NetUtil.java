...
public class NetUtil {
    ...
    public static final String LOCAL_IP = Ipv4Util.LOCAL_IP;
    /**
     * 默认最小端口，1024
     */
    ...
    public static int getUsableLocalPort(int minPort, int maxPort) {
        ...
        for (int i = minPort; i < maxPortExclude; i++) {
            randomPort = ToolRandom.randomInt(minPort, maxPortExclude);
        ...
        throw new UtilException(
                "Could not find an available port in the range [{}, {}] after {} attempts",
                minPort,
                maxPort,
                maxPort - minPort);
    }
    ...
    public static String hideIpPart(String ip) {
        return ToolString.builder(ip.length())
    ...
    public static Collection<NetworkInterface> getNetworkInterfaces() {
        ...
        return ToolCollection.addAll(new ArrayList<>(), networkInterfaces);
    }
    ...
    public static LinkedHashSet<String> localIpv4s() {
        final LinkedHashSet<InetAddress> localAddressList =
                localAddressList(t -> t instanceof Inet4Address);
    ...
    public static LinkedHashSet<String> localIpv6s() {
        final LinkedHashSet<InetAddress> localAddressList =
                localAddressList(t -> t instanceof Inet6Address);
    ...
    public static LinkedHashSet<InetAddress> localAddressList(
            Predicate<NetworkInterface> networkInterfaceFilter, Predicate<InetAddress> addressFilter) {
        Enumeration<NetworkInterface> networkInterfaces;
        try {
            networkInterfaces = NetworkInterface.getNetworkInterfaces();
        } catch (SocketException e) {
        ...
        while (networkInterfaces.hasMoreElements()) {
            ...
            if (networkInterfaceFilter != null
                    && false == networkInterfaceFilter.test(networkInterface)) {
                continue;
            }
     ...
     *
     * <p>参考：http://stackoverflow.com/questions/9481865/getting-the-ip-address-of-the-current-machine-using-java
     *
     ...
     * </pre>
     *
     * <p>此方法不会抛出异常，获取失败将返回{@code null}<br>
     *
     * <p>见：https://github.com/dromara/hutool/issues/428
     *
     * @return 本机网卡IP地址，获取失败返回{@code null}
     */
    public static InetAddress getLocalhost() {
        ...
        if (Whether.noEmpty(localAddressList)) {
    ...
    public static String getLocalHostName() {
        if (Whether.noBlank(localhostName)) {
    ...
    public static boolean isUnknown(String checkString) {
        return Whether.blank(checkString) || "unknown".equalsIgnoreCase(checkString);
    }
    ...
    public static List<String> getDnsInfo(String hostName, String... attrNames) {
        final String uri = ToolString.addPrefixIfNot(hostName, "dns:");
        final Attributes attributes = ToolJNDI.getAttributes(uri, attrNames);

        final List<String> infos = new ArrayList<>();

        for (Attribute attribute : AnyIter.ofEnumeration(true, attributes.getAll())) {
            try {
            ...
            while (netInterfaces.hasMoreElements()) {
                ...
                while (ips.hasMoreElements()) {
                    ...
                    if (!ip.isLoopbackAddress()
                            && PoolOfPattern.IPV4.matcher(ip.getHostAddress()).matches()) {
                        isFindIp = true;
                        break;
                    }
                }
     ...
     * @return IP对应的long值
     */
    public static int Ipv4StringToLong(String ipv4) {
        if (!PoolOfPattern.IPV4.matcher(ipv4).matches()) {
            throw new IllegalArgumentException("The input string is not in ipv4 format");
        }
     ...
     * @return long值对应的字符串
     */
    public static String longToIpv4String(int ipv4) {
        return (ipv4 >>> 24)
                + "."
                + ((ipv4 >>> 16) & 0xFF)
                + "."
     ...
     */
    public static long[] ipv6StringToLong(String ipString) {
     ...
     */
    public static String longToIpv6String(long... ipv6) {
        if (ipv6 == null || ipv6.length != 2) {
            throw new IllegalArgumentException(Arrays.toString(ipv6) + " is not an IPv6 address.");
        }
        StringBuilder sb = new StringBuilder(32);
        for (long numSlice : ipv6) {
            // 每个 long 保存四组
            for (int j = 0; j < 4; j++) {
                // 取最后 16 位
