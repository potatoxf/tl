...
public class Img implements Serializable {
    ...
    public Img scale(float scale) {
        ...
        if (ImgUtil.IMAGE_TYPE_PNG.equals(this.targetImageType)) {
            ...
            final double scaleDouble = ToolNumber.toDouble(scale);
            this.targetImage =
                    ImgUtil.transform(
            ...
            // 缩放后的图片宽
            final int width = ToolNumber.mul((Number) srcImg.getWidth(null), scale).intValue();
            // 缩放后的图片高
            final int height = ToolNumber.mul((Number) srcImg.getHeight(null), scale).intValue();
            scale(width, height);
        }
        return this;
    }
    ...
    public Img scale(int width, int height, int scaleType) {
        ...
        if (ImgUtil.IMAGE_TYPE_PNG.equals(this.targetImageType)) {
            ...
            final double sx = ToolNumber.div(width, srcWidth); // 宽度缩放比
            final double sy = ToolNumber.div(height, srcHeight); // 高度缩放比
            this.targetImage =
                    ImgUtil.transform(
    ...
    public Img scale(int width, int height, Color fixedColor) {
        ...
        double heightRatio = ToolNumber.div(height, srcHeight);
        double widthRatio = ToolNumber.div(width, srcWidth);

        // 浮点数之间的等值判断,基本数据类型不能用==比较,包装数据类型不能用equals来判断。
        if (ToolNumber.equals(heightRatio, widthRatio)) {
    ...
    public Img cut(Rectangle rectangle) {
        ...
        final ImageFilter cropFilter =
                new CropImageFilter(rectangle.x, rectangle.y, rectangle.width, rectangle.height);
    ...
    public Img round(double arc) {
        ...
        // 通过弧度占比计算弧度
        arc = ToolNumber.mul(arc, Math.min(width, height));
    ...
    public Img gray() {
        this.targetImage =
                ImgUtil.colorConvert(ColorSpace.getInstance(ColorSpace.CS_GRAY), getValidSrcBufferedImg());
        return this;
    }
    ...
    public Img pressText(String pressText, Color color, Font font, Point point, float alpha) {
        final BufferedImage targetImage =
                ImgUtil.toBufferedImage(getValidSrcImg(), this.targetImageType);
     ...
     * @return 处理后的图像
     */
    public Img pressTextFull(
            String pressText, Color color, Font font, int lineHeight, int degree, float alpha) {
        final BufferedImage targetImage =
                ImgUtil.toBufferedImage(getValidSrcImg(), this.targetImageType);
    ...
    public Img pressImage(Image pressImg, Rectangle rectangle, float alpha) {
        ...
        this.targetImage =
                draw(ImgUtil.toBufferedImage(targetImg, this.targetImageType), pressImg, rectangle, alpha);
        return this;
    }
    ...
    public Img rotate(int degree) {
        ...
        final BufferedImage targetImg =
                new BufferedImage(rectangle.width, rectangle.height, getTypeInt());
    ...
    public Img stroke(Color color, Stroke stroke) {
        ...
        g.setColor(Safe.value(color, Color.BLACK));
        if (null != stroke) {
            g.setStroke(stroke);
        }
    ...
    private Image getValidSrcImg() {
        return Safe.value(this.targetImage, this.srcImage);
    }
