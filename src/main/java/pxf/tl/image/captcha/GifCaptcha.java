...
public class GifCaptcha extends AbstractCaptcha {
    ...
    /**
     * 设置图像的颜色量化(转换质量 由GIF规范允许的最大256种颜色)。 低的值(最小值= 1)产生更好的颜色,但处理显著缓慢。 10是默认,并产生良好的颜色而且有以合理的速度。
     * 值更大(大于20)不产生显著的改善速度
     *
     * @param quality 大于1
     * @return this
    ...
    private BufferedImage graphicsImage(char[] chars, Color[] fontColor, char[] words, int flag) {
        ...
        // 利用指定颜色填充背景
        g2d.setColor(Safe.value(this.background, Color.WHITE));
        ...
        for (int i = 0; i < chars.length; i++) {
            ...
            g2d.drawOval(
                    ToolRandom.randomInt(width),
                    ToolRandom.randomInt(height),
                    ToolRandom.randomInt(5, 30),
                    5 + ToolRandom.randomInt(5, 30)); // 绘制椭圆边框
