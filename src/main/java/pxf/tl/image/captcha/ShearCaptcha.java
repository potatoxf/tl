...
import pxf.tl.image.GraphicsUtil;
import pxf.tl.image.ImgUtil;
...
public class ShearCaptcha extends AbstractCaptcha {
    ...
    public Image createImage(String code) {
        ...
        final Graphics2D g =
                GraphicsUtil.createGraphics(image, Safe.value(this.background, Color.WHITE));

        // 画字符串
        drawString(g, code);

        // 扭曲
        shear(g, this.width, this.height, Safe.value(this.background, Color.WHITE));
        // 画干扰线
        drawInterfere(
                g,
                0,
                ToolRandom.randomInt(this.height) + 1,
                this.width,
                ToolRandom.randomInt(this.height) + 1,
                this.interfereCount,
                ImgUtil.randomColor());
    ...
    private void shearX(Graphics g, int w1, int h1, Color color) {
        ...
        int period = ToolRandom.randomInt(this.width);

        int frames = 1;
        int phase = ToolRandom.randomInt(2);
    ...
    private void shearY(Graphics g, int w1, int h1, Color color) {
        ...
        int period = ToolRandom.randomInt(this.height >> 1);
