 ...
 * @author potatoxf
 */
public class Setting extends AbsSetting implements ExtendMap<String, String> {
    /**
     * 默认字符集
     */
     ...
     * @param isUseVariable 是否使用变量
     */
    public Setting(String path, Charsets charsets, boolean isUseVariable) {
        Assert.notBlank(path, "Blank setting path !");
        this.init(ResourceUtil.getResourceObj(path), charsets, isUseVariable);
    }
     ...
     * @param isUseVariable 是否使用变量
     */
    public Setting(File configFile, Charsets charsets, boolean isUseVariable) {
        Assert.notNull(configFile, "Null setting file define!");
        this.init(new FileResource(configFile), charsets, isUseVariable);
    }
     ...
     * @param isUseVariable 是否使用变量
     */
    public Setting(String path, Class<?> clazz, Charsets charsets, boolean isUseVariable) {
        Assert.notBlank(path, "Blank setting path !");
        this.init(new ClassPathResource(path, clazz), charsets, isUseVariable);
    }
     ...
     * @param isUseVariable 是否使用变量
     */
    public Setting(URL url, Charsets charsets, boolean isUseVariable) {
        Assert.notNull(url, "Null setting url define!");
        this.init(new UrlResource(url), charsets, isUseVariable);
    }
     ...
     * @param isUseVariable 是否使用变量
     */
    public Setting(Resource resource, Charsets charsets, boolean isUseVariable) {
        this.init(resource, charsets, isUseVariable);
    }
     ...
     * @return 成功初始化与否
     */
    public boolean init(Resource resource, Charsets charsets, boolean isUseVariable) {
    ...
    public synchronized boolean load() {
        if (null == this.settingLoader) {
            settingLoader = new SettingLoader(this.groupedMap, this.charsets, this.isUseVariable);
        }
    ...
    public void autoLoad(boolean autoReload, Consumer<Boolean> callback) {
        if (autoReload) {
            ...
            ToolLog.debug(LOGGER, () -> "Auto load for [%s] listenning...", this.resource.getUrl());
        } else {
        ...
        for (Entry<String, LinkedHashMap<String, String>> groupEntry : this.groupedMap.entrySet()) {
            ...
            for (Entry<String, String> entry : groupEntry.getValue().entrySet()) {
                properties.setProperty(
                        Whether.empty(group) ? entry.getKey() : group + PoolOfCharacter.DOT + entry.getKey(),
                        entry.getValue());
            }
        }
        return properties;
    ...
    public List<String> getGroups() {
        return New.list(false, this.groupedMap.keySet());
    }
     ...
     * @return this
     */
    public Setting setCharset(Charsets charsets) {
        this.charsets = charsets;
        return this;
    }
    ...
    public boolean equals(Object obj) {
        ...
        } else if (false == charsets.equals(other.charsets)) {
            return false;
        }
