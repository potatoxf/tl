...
public class GroupedMap extends LinkedHashMap<String, LinkedHashMap<String, String>> {
    ...
    public String get(String group, String key) {
        readLock.lock();
        try {
            LinkedHashMap<String, String> map = this.get(Safe.value(group));
    ...
    public String put(String group, String key, String value) {
        group = Safe.value(group).trim();
        writeLock.lock();
        try {
            final LinkedHashMap<String, String> valueMap =
                    this.computeIfAbsent(group, k -> new LinkedHashMap<>());
            this.size = -1;
     ...
     * @return 是否为空
     */
    public boolean empty(String group) {
        group = Safe.value(group).trim();
        readLock.lock();
        try {
            final LinkedHashMap<String, String> valueMap = this.get(group);
