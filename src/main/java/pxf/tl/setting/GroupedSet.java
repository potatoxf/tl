...
public class GroupedSet extends HashMap<String, LinkedHashSet<String>> {
     ...
     * @param charsets 字符集
     */
    public GroupedSet(Charsets charsets) {
     ...
     * @param charsets            字符集
     */
    public GroupedSet(String pathBaseClassLoader, Charsets charsets) {
        if (null == pathBaseClassLoader) {
            pathBaseClassLoader = ToolString.EMPTY;
        }

        final URL url = ToolURL.getURL(pathBaseClassLoader);
        if (url == null) {
            throw new RuntimeException(
                    ToolString.format("Can not find GroupSet file: [{}]", pathBaseClassLoader));
        }
        this.init(url, charsets);
    }
     ...
     * @param charsets   字符集
     */
    public GroupedSet(File configFile, Charsets charsets) {
        if (configFile == null) {
            throw new RuntimeException("Null GroupSet file!");
        }
        final URL url = ToolURL.getURL(configFile);
        this.init(url, charsets);
    }
     ...
     * @param charsets 字符集
     */
    public GroupedSet(String path, Class<?> clazz, Charsets charsets) {
        final URL url = ToolURL.getURL(path, clazz);
        if (url == null) {
            throw new RuntimeException(ToolString.format("Can not find GroupSet file: [{}]", path));
        }
        this.init(url, charsets);
    }
     ...
     * @param charsets 字符集
     */
    public GroupedSet(URL url, Charsets charsets) {
        if (url == null) {
    ...
    public GroupedSet(String pathBaseClassLoader) {
        this(pathBaseClassLoader, Charsets.UTF_8);
    }
     ...
     * @return 成功初始化与否
     */
    public boolean init(URL groupedSetUrl, Charsets charsets) {
        if (groupedSetUrl == null) {
            throw new RuntimeException("Null GroupSet url or charsets define!");
        }
        this.charsets = charsets;
    ...
    public boolean load(InputStream settingStream) throws IOException {
        ...
        try {
            reader = New.bufferedReader(settingStream, charsets);
            // 分组
            String group;
            ...
            while (true) {
                ...
                line = line.trim();
                // 跳过注释行和空行
                if (Whether.blank(line) || line.startsWith(COMMENT_FLAG_PRE)) {
                    // 空行和注释忽略
                    continue;
                } else if (line.startsWith(PoolOfString.BACKSLASH + COMMENT_FLAG_PRE)) {
                ...
                if (line.charAt(0) == GROUP_SURROUND[0]
                        && line.charAt(line.length() - 1) == GROUP_SURROUND[1]) {
                ...
                if (null == valueSet) {
                    ...
                    valueSet = new LinkedHashSet<>();
                    super.put(ToolString.EMPTY, valueSet);
                }
                valueSet.add(line);
            }
        } finally {
    ...
    public boolean contains(String group, String value, String... otherValues) {
        if (Whether.noEmpty(otherValues)) {
            // 需要测试多个值的情况
            final List<String> valueList = New.list(false, otherValues);
            valueList.add(value);
            return contains(group, valueList);
        } else {
            // 测试单个值
            final LinkedHashSet<String> valueSet = getValues(group);
            if (Whether.empty(valueSet)) {
                return false;
            }
    ...
    public boolean contains(String group, Collection<String> values) {
        final LinkedHashSet<String> valueSet = getValues(group);
        if (Whether.empty(values) || Whether.empty(valueSet)) {
            return false;
        }
