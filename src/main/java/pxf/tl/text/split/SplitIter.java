package pxf.tl.text.split;


import pxf.tl.help.Assert;
import pxf.tl.help.Whether;
import pxf.tl.iter.AbstractIter;
import pxf.tl.text.finder.TextFinder;
 ...
 * @author potatoxf
 */
public class SplitIter extends AbstractIter<String> implements Serializable {
    ...
    protected String doNext() {
        ...
        if (start < 0) {
            ...
            if (offset <= text.length()) {
                ...
                if (false == ignoreEmpty || false == Whether.empty(result)) {
                    // 返回非空串
        ...
        if (ignoreEmpty && Whether.empty(result)) {
            // 发现空串且需要忽略时，跳过之
            return doNext();
        }
    ...
    public List<String> toList(boolean trim) {
        return toList((str) -> trim ? ToolString.trim(str) : str);
    }
    ...
    public <T> List<T> toList(Function<String, T> mapping) {
        ...
        while (this.hasNext()) {
            ...
            if (ignoreEmpty && Whether.empty(apply)) {
                // 对于mapping之后依旧是String的情况，ignoreEmpty依旧有效
                continue;
            }
            result.add(apply);
        }
        if (Whether.empty(result)) {
