...
public class StrMatcher {
    ...
    private static List<String> parse(String pattern) {
        ...
        char pre;
        boolean inVar = false;
        StringBuilder part = ToolString.builder();
    ...
    public Map<String, String> match(String text) {
        final HashMap<String, String> result = New.map(true);
        int from = 0;
        String key = null;
        int to;
        for (String part : patterns) {
            if (ToolString.isWrap(part, "${", "}")) {
                // 变量
                key = ToolString.sub(part, 2, part.length() - 1);
            } else {
