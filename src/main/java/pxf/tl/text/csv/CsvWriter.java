...
public final class CsvWriter implements Closeable, Flushable, Serializable {
     ...
     * @param charsets 编码
     */
    public CsvWriter(String filePath, Charsets charsets) {
     ...
     * @param charsets 编码
     */
    public CsvWriter(File file, Charsets charsets) {
        this(file, charsets, false);
    }
     ...
     * @param isAppend 是否追加
     */
    public CsvWriter(String filePath, Charsets charsets, boolean isAppend) {
        this(FileUtil.file(filePath), charsets, isAppend);
    }
     ...
     * @param isAppend 是否追加
     */
    public CsvWriter(File file, Charsets charsets, boolean isAppend) {
        this(file, charsets, isAppend, null);
    }
     ...
     */
    public CsvWriter(String filePath, Charsets charsets, boolean isAppend, CsvWriteConfig config) {
        this(FileUtil.file(filePath), charsets, isAppend, config);
    }
     ...
     * @param config   写出配置，null则使用默认配置
     */
    public CsvWriter(File file, Charsets charsets, boolean isAppend, CsvWriteConfig config) {
        this(FileUtil.getWriter(file, charsets, isAppend), config);
    }
    ...
    public CsvWriter(Writer writer, CsvWriteConfig config) {
        ...
        this.config = Safe.value(config, CsvWriteConfig::defaultConfig);
    }
    // --------------------------------------------------------------------------------------------------- Constructor end
    ...
    public CsvWriter write(String[]... lines) throws IORuntimeException {
        return write(AnyIter.ofArray(lines));
    }
    ...
    public CsvWriter writeHeaderLine(String... fields) throws IORuntimeException {
        ...
        if (Whether.noEmpty(headerAlias)) {
            // 标题别名替换
            String alias;
    ...
    private void appendField(final String value) throws IOException {
        ...
        for (final char c : valueChars) {
            if (c == textDelimiter) {
                ...
                break;
            } else if (c == fieldSeparator || c == PoolOfCharacter.LF || c == PoolOfCharacter.CR) {
                // 包含分隔符或换行符需要包装符包装
