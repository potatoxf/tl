...
public class CsvReader extends CsvBaseReader implements Iterable<CsvRow>, Closeable {
     ...
     * @param config   配置项，null表示默认配置
     */
    public CsvReader(File file, Charsets charsets, CsvReadConfig config) {
        this(FileUtil.getReader(file, charsets), config);
    }
     ...
     * @param config   配置项，null表示默认配置
     */
    public CsvReader(Path path, Charsets charsets, CsvReadConfig config) {
        this(FileUtil.getReader(path, charsets), config);
    }
