...
public class StrJoiner implements Appendable, Serializable {
    ...
    // 当结果为空时默认返回的拼接结果
    private String emptyResult = PoolOfString.EMPTY;
     ...
     */
    public StrJoiner(
            Appendable appendable, CharSequence delimiter, CharSequence prefix, CharSequence suffix) {
        if (null != appendable) {
    ...
    public StrJoiner append(Object obj) {
        if (null == obj) {
            ...
            append(AnyIter.ofObject(obj, null));
        } else if (obj instanceof Iterator) {
            append(AnyIter.ofObject(obj, null));
        } else if (obj instanceof Iterable) {
            append(AnyIter.ofObject(obj, null));
        } else {
            append(ToolObject.toString(obj));
        }
        return this;
    }
    ...
    public <T> StrJoiner append(T[] array) {
        ...
        return append(AnyIter.ofArray(array));
    }
     ...
     * @return this
     */
    public <E> StrJoiner append(
            Iterable<E> iterable, Function<? super E, ? extends CharSequence> toStrFunc) {
        return append((Iterator<E>) AnyIter.ofIterable(iterable), toStrFunc);
    }
     ...
     */
    public <E> StrJoiner append(
            Iterator<E> iterator, Function<? super E, ? extends CharSequence> toStrFunc) {
        if (null != iterator) {
            while (iterator.hasNext()) {
        ...
        if (null == csq) {
            switch (this.nullMode) {
                case IGNORE:
                    return this;
                case TO_EMPTY:
                    csq = PoolOfString.EMPTY;
                    break;
                case NULL_STRING:
                    csq = PoolOfString.NULL;
                    endExclude = PoolOfString.NULL.length();
                    break;
            }
        }
        try {
            final Appendable appendable = prepare();
            if (wrapElement && Whether.noEmpty(this.prefix)) {
                appendable.append(prefix);
            }
            appendable.append(csq, startInclude, endExclude);
            if (wrapElement && Whether.noEmpty(this.suffix)) {
     ...
     */
    public int length() {
        return (this.appendable != null
                ? this.appendable.toString().length() + suffix.length()
    ...
    public String toString() {
        ...
        if (false == wrapElement && Whether.noEmpty(this.suffix)) {
            result += this.suffix;
        }
        return result;
    }
    ...
    private Appendable prepare() throws IOException {
        if (hasContent) {
            ...
            if (false == wrapElement && Whether.noEmpty(this.prefix)) {
    ...
    private void checkHasContent(Appendable appendable) {
        if (appendable instanceof CharSequence) {
            ...
            if (charSequence.length() > 0 && ToolString.endWith(charSequence, delimiter)) {
                this.hasContent = true;
            }
        } else {
            final String initStr = appendable.toString();
            if (Whether.noEmpty(initStr) && false == ToolString.endWith(initStr, delimiter)) {
                this.hasContent = true;
            }
        }
    }
