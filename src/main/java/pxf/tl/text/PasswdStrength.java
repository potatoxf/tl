...
public class PasswdStrength {
    ...
    public static int check(String passwd) {
        ...
        if (len > 4
                && countLetter(passwd, CHAR_TYPE.NUM) > 0
                && countLetter(passwd, CHAR_TYPE.SMALL_LETTER) > 0
                || countLetter(passwd, CHAR_TYPE.NUM) > 0
        ...
        if (len > 6
                && countLetter(passwd, CHAR_TYPE.NUM) >= 3
        ...
        if (len > 8
                && countLetter(passwd, CHAR_TYPE.NUM) >= 2
        ...
        if (ToolString.isNumeric(passwd)
                && ("01234567890".indexOf(passwd) > 0 || "09876543210".indexOf(passwd) > 0)) {
            level--;
        }

        if (countLetter(passwd, CHAR_TYPE.NUM) == len
                || countLetter(passwd, CHAR_TYPE.SMALL_LETTER) == len
        ...
        if (len % 2 == 0) { // aaabbb
            ...
            if (ToolString.isCharEquals(part1) && ToolString.isCharEquals(part2)) {
                level--;
            }
        }
        if (len % 3 == 0) { // ababab
        ...
        if (ToolString.isNumeric(passwd) && len >= 6 && len <= 8) { // 19881010 or 881010
            int year = 0;
            if (len == 8 || len == 6) {
        ...
        if (ToolString.isCharEquals(passwd)) {
            level = 0;
        }
