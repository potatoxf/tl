...
import pxf.tl.concurrent.ExecutorBuilder;
import pxf.tl.concurrent.ThreadFactoryBuilder;
import pxf.tl.help.Whether;
import pxf.tl.setting.Setting;
import pxf.tl.text.cron.listener.TaskListener;
import pxf.tl.text.cron.listener.TaskListenerManager;
import pxf.tl.text.cron.pattern.CronPattern;
import pxf.tl.text.cron.task.InvokeTask;
import pxf.tl.text.cron.task.RunnableTask;
 ...
 * @author potatoxf
 */
public class Scheduler implements Serializable, Sized {
    ...
    public Scheduler schedule(Setting cronSetting) {
        if (Whether.noEmpty(cronSetting)) {
            String group;
            for (Entry<String, LinkedHashMap<String, String>> groupedEntry :
                    cronSetting.getGroupedMap().entrySet()) {
                group = groupedEntry.getKey();
                for (Entry<String, String> entry : groupedEntry.getValue().entrySet()) {
                    String jobClass = entry.getKey();
                    if (Whether.noBlank(group)) {
                        jobClass = group + PoolOfCharacter.DOT + jobClass;
                    }
                    final String pattern = entry.getValue();
                    ToolLog.debug(LOGGER, () -> "Load job: %s %s", pattern, jobClass);
                    try {
                        // issue#I5E7BM@Gitee，自定义ID避免重复从配置文件加载
                        schedule("id_" + jobClass, pattern, new InvokeTask(jobClass));
                    } catch (Exception e) {
                        ToolLog.error(LOGGER, e, () -> "Schedule [{}] [{}] error!", pattern, jobClass);
                    }
                }
            }
        }
