...
public class PatternMatcher {
    ...
    private static boolean matchDayOfMonth(
            PartMatcher matcher, int dayOfMonth, int month, boolean isLeapYear) {
    ...
    private boolean match(
            int second, int minute, int hour, int dayOfMonth, int month, int dayOfWeek, int year) {
     ...
     *
     * <ul>
     *   <li>如果此部分下个值不变，获取下一个部分
     *   <li>如果此部分下个值大于给定值，以下所有值置为最小值
     *   <li>如果此部分下个值小于给定值，回退到上一个值获取下一个新值，之后的值置为最小值
     * </ul>
     *
     * <pre>
     *        秒 分 时 日 月 周 年
     ...
     *
     * <ul>
     *   <li>月在表达式中从1开始，但是{@link Calendar}中是从0开始的，需要-1
     *   <li>周在表达式中从0开始（0表示周日），但是{@link Calendar}中是从1开始的（1表示周日），需要+1
     * </ul>
     *
