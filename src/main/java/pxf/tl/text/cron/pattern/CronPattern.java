...
import pxf.tl.text.cron.pattern.matcher.PatternMatcher;
import pxf.tl.text.cron.pattern.parser.PatternParser;
 ...
 *
 * <ol>
 *   <li><strong>分</strong> ：范围：0~59
 *   <li><strong>时</strong> ：范围：0~23
 *   <li><strong>日</strong> ：范围：1~31，<strong>"L"</strong> 表示月的最后一天
 *   <li><strong>月</strong> ：范围：1~12，同时支持不区分大小写的别名："jan","feb", "mar", "apr", "may","jun", "jul",
 *       "aug", "sep","oct", "nov", "dec"
 *   <li><strong>周</strong> ：范围：0 (Sunday)~6(Saturday)，7也可以表示周日，同时支持不区分大小写的别名："sun","mon", "tue",
 *       "wed", "thu","fri", "sat"，<strong>"L"</strong> 表示周六
 * </ol>
 *
 * <p>为了兼容Quartz表达式，同时支持6位和7位表达式，其中：<br>
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>当定时任务运行到的时间匹配这些表达式后，任务被启动。<br>
 * 注意：
 *
 * <pre>
 ...
 *   <li><strong>cronA | cronB</strong> ：表示多个定时表达式
 * </ul>
 * <p>
 * 注意：在每一个子表达式中优先级：
 *
 ...
 * </pre>
 *
 * <p>例如 2,3,6/3中，由于“/”优先级高，因此相当于2,3,(6/3)，结果与 2,3,6等价<br>
 * <br>
 *
 * <p>一些例子：
 *
 * <ul>
 *   <li><strong>5 * * * *</strong> ：每个点钟的5分执行，00:05,01:05……
 *   <li><strong>* * * * *</strong> ：每分钟执行
 *   <li><strong>*&#47;2 * * * *</strong> ：每两分钟执行
 *   <li><strong>* 12 * * *</strong> ：12点的每分钟执行
 *   <li><strong>59 11 * * 1,2</strong> ：每周一和周二的11:59执行
 *   <li><strong>3-18&#47;5 * * * *</strong> ：3~18分，每5分钟执行一次，即0:03, 0:08, 0:13, 0:18, 1:03, 1:08……
 * </ul>
 *
 * @author potatoxf
 */
public class CronPattern {
    ...
    private Calendar nextMatchAfter(int[] values, TimeZone zone) {
        ...
        // 返回匹配到的最早日期
        return ToolCollection.min(nextMatches);
    }
}
