package pxf.tl.text.cron.pattern.parser;


import pxf.tl.api.PoolOfCharacter;
import pxf.tl.date.Month;
import pxf.tl.date.Week;
import pxf.tl.text.cron.CronException;
import pxf.tl.text.cron.pattern.Part;
import pxf.tl.text.cron.pattern.matcher.*;
 ...
 *
 * <ul>
 *   <li><strong>*</strong> ：表示匹配这个位置所有的时间
 *   <li><strong>?</strong> ：表示匹配这个位置任意的时间（与"*"作用一致）
 *   <li><strong>L</strong> ：表示匹配这个位置允许的最大值
 *   <li><strong>*&#47;2</strong> ：表示间隔时间，例如在分上，表示每两分钟，同样*可以使用数字列表代替，逗号分隔
 *   <li><strong>2-8</strong> ：表示连续区间，例如在分上，表示2,3,4,5,6,7,8分
 *   <li><strong>2,3,5,8</strong> ：表示列表
 *   <li><strong>wed</strong> ：表示周别名
 *   <li><strong>jan</strong> ：表示月别名
 * </ul>
 *
 * @author potatoxf
 */
public class PartParser {
     ...
     *
     * <ul>
     *   <li>* 或者 ? 返回{@link AlwaysTrueMatcher}
     *   <li>{@link Part#DAY_OF_MONTH} 返回{@link DayOfMonthMatcher}
     *   <li>{@link Part#YEAR} 返回{@link YearValueMatcher}
     *   <li>其他 返回{@link BoolArrayMatcher}
     * </ul>
     *
     * @param value 表达式
     ...
     *
     * <ul>
     *   <li><strong>a</strong> 或 <strong>*</strong>
     *   <li><strong>a,b,c,d</strong>
     * </ul>
     *
     * @param value 子表达式值
     * @return 值列表
    ...
    private List<Integer> parseArray(String value) {
        ...
        final List<String> parts = ToolString.splitTrim(value, PoolOfCharacter.COMMA);
        for (String part : parts) {
            ToolCollection.addAllIfNotContains(values, parseStep(part));
        }
        return values;
    }
     ...
     * <ul>
     *   <li><strong>a</strong> 或 <strong>*</strong>
     *   <li><strong>a&#47;b</strong> 或 <strong>*&#47;b</strong>
     *   <li><strong>a-b/2</strong>
     * </ul>
     *
     * @param value 表达式值
     * @return List
    ...
    private List<Integer> parseStep(String value) {
        final List<String> parts = ToolString.splitTrim(value, PoolOfCharacter.SLASH);
        int size = parts.size();
    ...
    private List<Integer> parseRange(String value, int step) {
        ...
        // Range模式
        List<String> parts = ToolString.splitTrim(value, '-');
        int size = parts.size();
        if (size == 1) { // 普通值
            final int v1 = parseNumber(value);
            if (step > 0) { // 类似 20/2的形式
                ToolNumber.appendRange(v1, part.getMax(), step, results);
            } else {
                results.add(v1);
            }
            ...
            if (v1 < v2) { // 正常范围，例如：2-5
                ToolNumber.appendRange(v1, v2, step, results);
            } else if (v1 > v2) { // 逆向范围，反选模式，例如：5-2
                ToolNumber.appendRange(v1, part.getMax(), step, results);
                ToolNumber.appendRange(part.getMin(), v2, step, results);
     ...
     *
     * <ul>
     *   <li><strong>L 表示最大值</strong>
     *   <li>{@link Part#MONTH}和{@link Part#DAY_OF_WEEK}别名
     * </ul>
     *
     * @param name 别名
     * @return 解析int值
