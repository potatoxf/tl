...
public class NumberWordFormatter {
    ...
    private static final String[] NUMBER_TEEN =
            new String[]{
                    "TEN",
                    "ELEVEN",
                    "TWELVE",
    ...
    public static String formatSimple(long value, boolean isTwo) {
        ...
        return String.format("%s%s", ToolNumber.decimalFormat("#.##", res), NUMBER_SUFFIX[index]);
    }
    ...
    private static String format(String x) {
        ...
        String lstrrev = ToolString.reverse(lstr); // 对左边的字串取反
        ...
        for (int i = 0; i < lstrrev.length() / 3; i++) {
            a[i] = ToolString.reverse(lstrrev.substring(3 * i, 3 * i + 3)); // 截取第一个三位
