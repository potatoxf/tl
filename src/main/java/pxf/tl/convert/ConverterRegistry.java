...
import pxf.tl.util.ToolServiceLoader;
 ...
 *
 * <p>在此类中，存放着默认转换器和自定义转换器，默认转换器是Hutool中预定义的一些转换器，自定义转换器存放用户自定的转换器。
 *
 * @author potatoxf
 */
public class ConverterRegistry implements Serializable {
    ...
    private void putCustomBySpi() {
        ToolServiceLoader.load(Converter.class)
                .forEach(
                        converter -> {
                            try {
                                Type type = ToolClass.getTypeArgument(ToolClass.getClass(converter));
                                if (null != type) {
    ...
    public ConverterRegistry putCustom(Type type, Class<? extends Converter<?>> converterClass) {
        return putCustom(type, ToolBytecode.createInstance(converterClass));
    }
    ...
    public <T> T convert(Type type, Object value, T defaultValue, boolean isCustomFirst)
        ...
        if (ToolClass.isUnknown(type) && null == defaultValue) {
            // 对于用户不指定目标类型的情况，返回原值
            return (T) value;
        }
        if (Whether.nvl(value)) {
            return defaultValue;
        }
        if (ToolClass.isUnknown(type)) {
        ...
        Class<T> rowType = (Class<T>) ToolBytecode.getClass(type);
        if (null == rowType) {
        ...
        throw new ConvertException(
                "Can not Converter from [{}] to [{}]", value.getClass().getName(), type.getTypeName());
    }
    ...
    private ConverterRegistry defaultConverter() {
        ...
        defaultConverterMap.put(
                AtomicInteger.class, new NumberConverter(AtomicInteger.class)); // since 3.0.8
        ...
        // Reference
        defaultConverterMap.put(
                WeakReference.class, new ReferenceConverter(WeakReference.class)); // since 3.0.8
        defaultConverterMap.put(
                SoftReference.class, new ReferenceConverter(SoftReference.class)); // since 3.0.8
        ...
        defaultConverterMap.put(
                StackTraceElement.class, new StackTraceElementConverter()); // since 4.5.2
        defaultConverterMap.put(Optional.class, new OptionalConverter()); // since 5.0.0
        defaultConverterMap.put(Optional.class, new OptConverter()); // since 5.7.16
