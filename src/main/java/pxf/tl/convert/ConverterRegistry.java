...
public class ConverterRegistry implements Serializable {
    ...
    private void putCustomBySpi() {
        ToolServiceLoader.load(Converter.class)
                .forEach(
                        converter -> {
                            try {
                                Type type = ToolBytecode.getTypeArgument(ToolBytecode.getClass(converter));
                                if (null != type) {
    ...
    public ConverterRegistry putCustom(Type type, Class<? extends Converter<?>> converterClass) {
        return putCustom(type, ToolBytecode.createInstanceSilent(converterClass));
    }
    ...
    public <T> T convert(Type type, Object value, T defaultValue, boolean isCustomFirst)
        ...
        if (ToolBytecode.isUnknown(type) && null == defaultValue) {
            // 对于用户不指定目标类型的情况，返回原值
            return (T) value;
        }
        ...
        if (ToolBytecode.isUnknown(type)) {
