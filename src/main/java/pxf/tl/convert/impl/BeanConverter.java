...
import pxf.tl.collection.map.MapProxy;
...
public class BeanConverter<T> extends AbstractConverter<T> {
    ...
    public BeanConverter(Type beanType, CopyOptions copyOptions) {
        ...
        this.beanClass = (Class<T>) ToolBytecode.getClass(beanType);
    ...
    protected T convertInternal(Object value) {
        if (value instanceof Map
            ...
            return BeanCopier.create(
                            value,
                            ToolBytecode.createInstanceIfPossible(this.beanClass),
                            this.beanType,
                            this.copyOptions)
                    .copy();
        } else if (value instanceof byte[]) {
            // 尝试反序列化
            return ToolObject.deserialize((byte[]) value);
        }
