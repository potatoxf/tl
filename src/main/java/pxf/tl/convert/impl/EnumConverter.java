...
public class EnumConverter extends AbstractConverter<Object> {
    ...
    private static final WeakConcurrentMap<Class<?>, Map<Class<?>, Method>> VALUE_OF_METHOD_CACHE =
     ...
     *
     * <ul>
     *   <li>如果实现{@link EnumItem}接口，则调用fromInt或fromStr转换
     *   <li>找到类似转换的静态方法调用实现转换且优先使用
     *   <li>约定枚举类应该提供 valueOf(String) 和 valueOf(Integer)用于转换
     *   <li>oriInt /name 转换托底
     * </ul>
     *
     * @param value     被转换的值
    ...
    protected static Enum tryConvertEnum(Object value, Class enumClass) {
        ...
        if (EnumItem.class.isAssignableFrom(enumClass)) {
            final EnumItem first = (EnumItem) ToolEnum.getEnumAt(enumClass, 0);
            if (null != first) {
        ...
        try {
            ...
            if (Whether.noEmpty(methodMap)) {
                final Class<?> valueClass = value.getClass();
                for (Map.Entry<Class<?>, Method> entry : methodMap.entrySet()) {
                    if (ToolClass.isAssignable(entry.getKey(), valueClass)) {
                        return ToolBytecode.invokeSilent(null, entry.getValue(), value);
                    }
                }
            }
        ...
        if (value instanceof Integer) {
            enumResult = ToolEnum.getEnumAt(enumClass, (Integer) value);
        } else if (value instanceof String) {
            try {
    ...
    private static Map<Class<?>, Method> getMethodMap(Class<?> enumClass) {
        return VALUE_OF_METHOD_CACHE.computeIfAbsent(
                enumClass,
                (key) ->
                        Arrays.stream(enumClass.getMethods())
                                .filter(Whether::staticModifier)
