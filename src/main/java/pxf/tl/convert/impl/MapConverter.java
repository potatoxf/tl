...
public class MapConverter extends AbstractConverter<Map<?, ?>> {
    ...
    public MapConverter(Type mapType) {
        this(mapType, ToolClass.getTypeArgument(mapType, 0), ToolClass.getTypeArgument(mapType, 1));
    }
    ...
    protected Map<?, ?> convertInternal(Object value) {
        ...
        if (value instanceof Map) {
            ...
            if (valueClass.equals(this.mapType)) {
                final Type[] typeArguments = ToolClass.getTypeArguments(valueClass);
    ...
    private void convertMapToMap(Map<?, ?> srcMap, Map<Object, Object> targetMap) {
        ...
        srcMap.forEach(
                (key, value) -> {
                    key = ToolClass.isUnknown(this.keyType) ? key : convert.convert(this.keyType, key);
                    value =
                            ToolClass.isUnknown(this.valueType) ? value : convert.convert(this.valueType, value);
