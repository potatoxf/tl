...
public abstract class AbstractConverter<T> implements Converter<T>, Serializable {
    ...
    public T convert(Object value, T defaultValue) {
        ...
        if (null == targetType && null == defaultValue) {
            throw new NullPointerException(
                    String.format(
                            "[type] and [defaultValue] are both null for Converter [%s], we can not know what type to convert !",
        ...
        if (null == defaultValue || targetType.isInstance(defaultValue)) {
            if (targetType.isInstance(value) && !Object.class.isAssignableFrom(targetType)) {
            ...
            throw new IllegalArgumentException(
                    String.format(
                            "Default value [%s](%s) is not the instance of [%s]",
                            defaultValue,
    ...
    protected String convertToStr(Object value) {
        ...
        } else if (Whether.arrayObj(value)) {
            return ToolArray.toString(value);
        } else if (ToolChar.isChar(value)) {
            // 对于ASCII字符使用缓存加速转换，减少空间创建
            return ToolChar.toString((char) value);
        }
        return value.toString();
    }
    ...
    public Class<T> getTargetType() {
        return (Class<T>) ToolClass.getTypeArgument(getClass());
    }
}
