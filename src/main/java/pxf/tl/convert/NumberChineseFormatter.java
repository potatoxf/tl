...
public class NumberChineseFormatter {
    ...
    private static final char[] DIGITS = {
            '零', '一', '壹', '二', '贰', '三', '叁', '四', '肆', '五', '伍', '六', '陆', '七', '柒', '八', '捌', '九', '玖'
    };
     ...
     * <p>而非：负壹拾贰元叁角贰分
     *
     * <p>共两点不同：1、(负数) 而非 负；2、圆 而非 元 2022/3/9
     *
     * @param amount           数字
     * @param isUseTraditional 是否使用繁体
    ...
    public static String format(
        ...
        // 负数
        if (amount < 0) {
            chineseStr.append(Whether.empty(negativeName) ? "负" : negativeName);
            amount = -amount;
        }
        ...
        if (!isMoneyMode || 0 != yuan) {
            ...
            if (isMoneyMode) {
                chineseStr.append(Whether.empty(unitName) ? "元" : unitName);
            }
        }
    ...
    public static String formatSimple(long amount) {
        ...
        if (amount < 1_0000_0000 && amount > -1_0000_0000) {
            res = ToolNumber.div(amount, 1_0000, 2) + "万";
        } else if (amount < 1_0000_0000_0000L && amount > -1_0000_0000_0000L) {
            res = ToolNumber.div(amount, 1_0000_0000, 2) + "亿";
        } else {
            res = ToolNumber.div(amount, 1_0000_0000_0000L, 2) + "万亿";
        }
        return res;
    }
     ...
     *
     * <p>如： “陆万柒仟伍佰伍拾陆元叁角贰分”返回“67556.32” “叁角贰分”返回“0.32”
     *
    ...
    public static BigDecimal chineseMoneyToNumber(String chineseMoneyAmount) {
        if (Whether.blank(chineseMoneyAmount)) {
            return null;
        }
        ...
        amount =
                amount.add(BigDecimal.valueOf(f).divide(BigDecimal.valueOf(100), RoundingMode.HALF_UP));
        return amount;
    }
    ...
    private static String longToChinese(long amount, boolean isUseTraditional) {
        ...
        if (Whether.noEmpty(chineseStr) && '零' == chineseStr.charAt(0)) {
    ...
    private static String thousandToChinese(int amountPart, boolean isUseTraditional) {
        ...
        for (int i = 0; temp > 0; i++) {
            ...
            if (digit == 0) { // 取到的数字为 0
                ...
                chineseStr.insert(
                        0, numberToChinese(digit, isUseTraditional) + getUnitName(i, isUseTraditional));
                lastIsZero = false;
            }
            temp = temp / 10;
        }
            ...
            if (num >= 0) {
                if (num == 0) {
                    ...
                    throw new IllegalArgumentException(
                            ToolString.format("Bad number '{}{}' at: {}", chinese.charAt(i - 1), c, i));
                }
                // 普通数字
                number = num;
            } else {
                ...
                if (null == unit) {
                    // 出现非法字符
                    throw new IllegalArgumentException(ToolString.format("Unknown unit '{}' at: {}", c, i));
                }
    ...
    private static int chineseToNumber(char chinese) {
        ...
        final int i = ToolArray.indexOf(DIGITS, chinese);
        if (i > 0) {
            return (i + 1) / 2;
        }
        return i;
