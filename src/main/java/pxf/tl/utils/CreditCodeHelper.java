 ...
 * @author potatoxf
 */
public class CreditCodeHelper {

    public static final Pattern CREDIT_CODE_PATTERN = PoolOfPattern.CREDIT_CODE;
    ...
    public static boolean isCreditCodeSimple(CharSequence creditCode) {
        if (Whether.blank(creditCode)) {
            return false;
        }
        return ToolRegex.isMatch(CREDIT_CODE_PATTERN, creditCode);
    }
    ...
    public static String randomCreditCode() {
        ...
        //
        for (int i = 0; i < 2; i++) {
            int num = ToolRandom.randomInt(BASE_CODE_ARRAY.length - 1);
            buf.append(Character.toUpperCase(BASE_CODE_ARRAY[num]));
        }
        for (int i = 2; i < 8; i++) {
            int num = ToolRandom.randomInt(10);
