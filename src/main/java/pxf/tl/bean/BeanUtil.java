...
import pxf.tl.bean.copier.BeanCopier;
 ...
 * Bean工具类
 *
 * <p>把一个拥有对属性进行set和get方法的类，我们就可以称之为JavaBean。
 *
 * @author potatoxf
 */
public class BeanUtil {
    ...
    public static boolean hasSetter(Class<?> clazz) {
        if (ToolBytecode.isNormalClass(clazz)) {
    ...
    public static boolean hasPublicField(Class<?> clazz) {
        if (ToolBytecode.isNormalClass(clazz)) {
            for (Field field : clazz.getFields()) {
                if (Whether.publicModifier(field) && !Whether.staticModifier(field)) {
                    // 非static的public字段
                    return true;
                }
            }
    ...
    public static BeanDesc getBeanDesc(Class<?> clazz) {
        return BeanDescCache.INSTANCE.getBeanDesc(clazz, (clz) -> new BeanDesc(clazz));
    }
    ...
    public static PropertyDescriptor[] getPropertyDescriptors(Class<?> clazz) throws BeanException {
        ...
        return ToolArray.filter(beanInfo.getPropertyDescriptors(), (Predicate<PropertyDescriptor>) propertyDescriptor -> {
            // 过滤掉getClass方法
    ...
    public static Map<String, PropertyDescriptor> getPropertyDescriptorMap(
            Class<?> clazz, boolean ignoreCase) throws BeanException {
        return BeanInfoCache.INSTANCE.getPropertyDescriptorMap(
                clazz, ignoreCase, (clz) -> internalGetPropertyDescriptorMap(clazz, ignoreCase));
    }
    ...
    public static PropertyDescriptor getPropertyDescriptor(
            Class<?> clazz, final String fieldName, boolean ignoreCase) throws BeanException {
     ...
     *
     * <ul>
     *   <li>Map: fieldNameOrIndex需为key，获取对应value
     *   <li>Collection: fieldNameOrIndex当为数字，返回index对应值，非数字遍历集合返回子bean对应name值
     *   <li>Array: fieldNameOrIndex当为数字，返回index对应值，非数字遍历数组返回子bean对应name值
     * </ul>
     *
     * @param bean             Bean对象
    ...
    public static Object getFieldValue(Object bean, String fieldNameOrIndex) {
        ...
        if (bean instanceof Map) {
            ...
            try {
                return Safe.atIdx((Collection<?>) bean, Integer.parseInt(fieldNameOrIndex), null);
            } catch (NumberFormatException e) {
                // 非数字，see pr#254@Gitee
                return ToolCollection.map(
                        (Collection<?>) bean, (beanEle) -> getFieldValue(beanEle, fieldNameOrIndex), false);
            }
        } else if (Whether.arrayObj(bean)) {
            try {
                return ToolArray.get(bean, Integer.parseInt(fieldNameOrIndex));
            } catch (NumberFormatException e) {
                // 非数字，see pr#254@Gitee
                return ToolArray.map(
                        bean, Object.class, (beanEle) -> getFieldValue(beanEle, fieldNameOrIndex));
            }
        } else { // 普通Bean对象
            return ToolBytecode.getFieldValueAsSafeSilent(bean, fieldNameOrIndex);
        }
    }
    ...
    public static void setFieldValue(Object bean, String fieldNameOrIndex, Object value) {
        if (bean instanceof Map) {
            ...
            ToolCollection.setOrPadding((List) bean, Convert.toInt(fieldNameOrIndex), value);
        } else if (Whether.arrayObj(bean)) {
            ToolArray.setOrAppend(bean, Convert.toInt(fieldNameOrIndex), value);
        } else {
            // 普通Bean对象
            ToolBytecode.setFieldValueAsSafeSilent(bean, fieldNameOrIndex, value);
        }
    }
    ...
    public static <T> T getProperty(Object bean, String expression) {
        if (null == bean || Whether.blank(expression)) {
            return null;
        }
    ...
    public static <T> T mapToBean(Map<?, ?> map, Class<T> beanClass, boolean isIgnoreError) {
        return fillBeanWithMap(map, ToolBytecode.createInstanceIfPossible(beanClass), isIgnoreError);
    }
     ...
     */
    @Deprecated
    public static <T> T mapToBeanIgnoreCase(
            Map<?, ?> map, Class<T> beanClass, boolean isIgnoreError) {
        return fillBeanWithMapIgnoreCase(
                map, ToolBytecode.createInstanceIfPossible(beanClass), isIgnoreError);
    }
    ...
    public static <T> T mapToBean(Map<?, ?> map, Class<T> beanClass, CopyOptions copyOptions) {
        return fillBeanWithMap(map, ToolBytecode.createInstanceIfPossible(beanClass), copyOptions);
    }
    ...
    public static <T> T mapToBean(
            Map<?, ?> map, Class<T> beanClass, boolean isToCamelCase, CopyOptions copyOptions) {
        return fillBeanWithMap(
                map, ToolBytecode.createInstanceIfPossible(beanClass), isToCamelCase, copyOptions);
    }
     ...
     * @return Bean
     */
    public static <T> T fillBeanWithMap(
            Map<?, ?> map, T bean, boolean isToCamelCase, boolean isIgnoreError) {
        return fillBeanWithMap(
                map, bean, isToCamelCase, CopyOptions.create().setIgnoreError(isIgnoreError));
    }
    ...
    public static <T> T fillBeanWithMapIgnoreCase(Map<?, ?> map, T bean, boolean isIgnoreError) {
        return fillBeanWithMap(
                map, bean, CopyOptions.create().setIgnoreCase(true).setIgnoreError(isIgnoreError));
    }
     ...
     */
    public static <T> T fillBeanWithMap(
            Map<?, ?> map, T bean, boolean isToCamelCase, CopyOptions copyOptions) {
    ...
    public static <T> T toBeanIgnoreCase(Object source, Class<T> clazz, boolean ignoreError) {
        return toBean(
                source, clazz, CopyOptions.create().setIgnoreCase(true).setIgnoreError(ignoreError));
    }
    ...
    public static <T> T toBean(Object source, Class<T> clazz, CopyOptions options) {
        return toBean(source, () -> ToolBytecode.createInstanceIfPossible(clazz), options);
    }
    ...
    public static <T> T toBean(
            Class<T> beanClass, ValueProvider<String> valueProvider, CopyOptions copyOptions) {
        if (null == beanClass || null == valueProvider) {
            return null;
        }
        return fillBean(ToolBytecode.createInstanceIfPossible(beanClass), valueProvider, copyOptions);
    }
    ...
    public static <T> T fillBean(
            T bean, ValueProvider<String> valueProvider, CopyOptions copyOptions) {
    ...
    public static Map<String, Object> beanToMap(Object bean, String... properties) {
        ...
        if (Whether.noEmpty(properties)) {
            ...
            final Set<String> propertiesSet = New.set(false, properties);
    ...
    public static Map<String, Object> beanToMap(
        ...
        return beanToMap(
                ...
                targetMap,
                ignoreNullValue,
                key -> isToUnderlineCase ? Safe.toUnderlineCase(key,false) : key);
    }
    ...
    public static Map<String, Object> beanToMap(
        ...
        return BeanCopier.create(
                        bean,
                        targetMap,
                        CopyOptions.create().setIgnoreNullValue(ignoreNullValue).setFieldNameEditor(keyEditor))
                .copy();
    }
     ...
     */
    public static Map<String, Object> beanToMap(
            Object bean, Map<String, Object> targetMap, CopyOptions copyOptions) {
        if (null == bean) {
            return null;
        }
    ...
    public static <T> T copyProperties(Object source, Class<T> tClass, String... ignoreProperties) {
        ...
        T target = ToolBytecode.createInstanceIfPossible(tClass);
    ...
    public static void copyProperties(Object source, Object target, CopyOptions copyOptions) {
        ...
        BeanCopier.create(source, target, Safe.value(copyOptions, CopyOptions::create)).copy();
    }
    ...
    public static <T> List<T> copyToList(
            Collection<?> collection, Class<T> targetType, CopyOptions copyOptions) {
        if (null == collection) {
            return null;
        }
        ...
        return collection.stream()
                .map(
                        (source) -> {
                            final T target = ToolBytecode.createInstanceIfPossible(targetType);
    ...
    public static boolean isMatchName(Object bean, String beanClassName, boolean isSimple) {
        if (null == bean || Whether.blank(beanClassName)) {
            return false;
        }
        return ToolBytecode.getClassName(bean, isSimple)
                .equals(isSimple ? ToolString.upperFirst(beanClassName) : beanClassName);
    }
    ...
    public static <T> T edit(T bean, Editor<Field> editor) {
        ...
        final Field[] fields = ToolBytecode.getFields(bean.getClass(), field -> !Whether.staticModifier(field));
     ...
     *
     * <p>通常bean直接用来绑定页面的input，用户的输入可能首尾存在空格，通常保存数据库前需要把首尾空格去掉
     *
     * @param <T>          Bean类型
     * @param bean         Bean对象
    ...
    public static <T> T trimStrFields(T bean, String... ignoreFields) {
        return edit(
                bean,
                (field) -> {
                    if (ignoreFields != null && ToolArray.containsIgnoreCase(ignoreFields, field.getName())) {
                        // 不处理忽略的Fields
                        return field;
                    }
                    if (String.class.equals(field.getType())) {
                        // 只有String的Field才处理
                        final String val = (String) ToolBytecode.getFieldValueAsSafeSilent(bean, field);
                        if (null != val) {
                            final String trimVal = ToolString.trim(val);
                            if (false == val.equals(trimVal)) {
                                // Field Value不为null，且首尾有空格才处理
                                ToolBytecode.setFieldValueAsSafeSilent(bean, field, trimVal);
                            }
                        }
                    }
                    return field;
    ...
    public static boolean isNotEmpty(Object bean, String... ignoreFiledNames) {
        return !empty(bean, ignoreFiledNames);
    }
     ...
     */
    public static boolean empty(Object bean, String... ignoreFiledNames) {
        if (null != bean) {
            for (Field field : ToolBytecode.getNoStaticFields(bean.getClass())) {
                if ((!ToolArray.contains(ignoreFiledNames, field.getName()))
                        && null != ToolBytecode.getFieldValueAsSafeSilent(bean, field)) {
                    return false;
                }
            }
        }
    ...
    public static boolean hasNullField(Object bean, String... ignoreFiledNames) {
        ...
        for (Field field : ToolBytecode.getNoStaticFields(bean.getClass())) {
            if ((false == ToolArray.contains(ignoreFiledNames, field.getName()))
                    && null == ToolBytecode.getFieldValueAsSafeSilent(bean, field)) {
                return true;
            }
        }
     ...
     *
     * <ul>
     *   <li>getXxxx获取为xxxx，如getName得到name。
     *   <li>setXxxx获取为xxxx，如setName得到name。
     *   <li>isXxxx获取为xxxx，如isName得到name。
     *   <li>其它不满足规则的方法名抛出{@link IllegalArgumentException}
     * </ul>
     *
    ...
    public static String getFieldName(String getterOrSetterName) {
        if (getterOrSetterName.startsWith("get") || getterOrSetterName.startsWith("set")) {
            return ToolString.removePreAndLowerFirst(getterOrSetterName, 3);
        } else if (getterOrSetterName.startsWith("is")) {
            return ToolString.removePreAndLowerFirst(getterOrSetterName, 2);
        } else {
     ...
     * @author potatoxf
     */
    public static boolean isCommonFieldsEqual(
        ...
        for (String field : sourceFields) {
            if (ToolObject.notEqual(sourceFieldsMap.get(field), targetFieldsMap.get(field))) {
                return false;
            }
        }
