...
import pxf.tl.annotation.PropIgnore;
...
public class PropDesc {
    ...
    public PropDesc(Field field, Method getter, Method setter) {
        this.field = field;
        this.getter = ToolBytecode.setAccessible(getter);
        this.setter = ToolBytecode.setAccessible(setter);
    }
    ...
    public String getFieldName() {
        return ToolBytecode.getFieldUniqueKey(this.field);
    }
    ...
    public Type getFieldType() {
        if (null != this.field) {
            return ToolBytecode.getType(this.field);
        }
    ...
    public Class<?> getFieldClass() {
        if (null != this.field) {
            return ToolBytecode.getClass(this.field);
        }
    ...
    public boolean isReadable(boolean checkTransient) {
        ...
        if (null == this.getter && false == Whether.staticModifier(this.field)) {
            return false;
        }
    ...
    public Object getValue(Object bean) {
        if (null != this.getter) {
            return ToolBytecode.invokeSilent(bean, this.getter);
        } else if (Whether.publicModifier(this.field)) {
            return ToolBytecode.getFieldValueAsSafeSilent(bean, this.field);
        }
    ...
    public boolean isWritable(boolean checkTransient) {
        // 检查是否有getter方法或是否为public修饰
        if (null == this.setter && false == Whether.staticModifier(this.field)) {
            return false;
        }
    ...
    public PropDesc setValue(Object bean, Object value) {
        if (null != this.setter) {
            ToolBytecode.invokeSilent(bean, this.setter, value);
        } else if (Whether.publicModifier(this.field)) {
            ToolBytecode.setFieldValueAsSafeSilent(bean, this.field, value);
        }
        return this;
    }
    ...
    public PropDesc setValue(
            Object bean, Object value, boolean ignoreNull, boolean ignoreError, boolean override) {
    ...
    private Type findPropType(Method getter, Method setter) {
        Type type = null;
        if (null != getter) {
            type = ToolBytecode.getReturnType(getter);
        }
        if (null == type && null != setter) {
            type = ToolBytecode.getParamType(setter, 0);
        }
        return type;
    }
    ...
    private Class<?> findPropClass(Method getter, Method setter) {
        Class<?> type = null;
        if (null != getter) {
            type = ToolBytecode.getMethodReturnClass(getter);
        }
        if (null == type && null != setter) {
            type = ToolBytecode.getFirstParamClass(setter);
        }
        return type;
    }
    ...
    private boolean isTransientForGet() {
        boolean isTransient = Whether.withModifier(this.field, Modifier.TRANSIENT);

        // 检查Getter方法
        if (false == isTransient && null != this.getter) {
            isTransient = Whether.withModifier(this.getter, Modifier.TRANSIENT);
    ...
    private boolean isTransientForSet() {
        ...
        if (false == isTransient && null != this.setter) {
            ...
            isTransient = Whether.withModifier(this.setter, Modifier.TRANSIENT);
