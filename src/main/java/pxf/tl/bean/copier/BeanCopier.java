...
public class BeanCopier<T> implements Copier<T>, Serializable {
    ...
    public BeanCopier(Object source, T target, Type targetType, CopyOptions copyOptions) {
        ...
        if (source instanceof Map) {
            if (target instanceof Map) {
                ...
                copier =
                        (Copier<T>)
                                new MapToMapCopier((Map<?, ?>) source, (Map<?, ?>) target, targetType, copyOptions);
            } else {
            ...
            //noinspection unchecked
            copier =
                    new ValueProviderToBeanCopier<>(
                            (ValueProvider<String>) source, target, targetType, copyOptions);
        } else {
            if (target instanceof Map) {
                //noinspection unchecked
                copier =
                        (Copier<T>) new BeanToMapCopier(source, (Map<?, ?>) target, targetType, copyOptions);
            } else {
     ...
     * @return BeanCopier
     */
    public static <T> BeanCopier<T> create(
            Object source, T target, Type destType, CopyOptions copyOptions) {
