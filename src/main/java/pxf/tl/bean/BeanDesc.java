...
public class BeanDesc implements Serializable {
    ...
    public Map<String, PropDesc> getPropMap(boolean ignoreCase) {
        return ignoreCase ? new CaseInsensitiveMap<>(propMap) : this.propMap;
    }
    ...
    private BeanDesc init() {
        final Method[] gettersAndSetters =
                ToolBytecode.getMethods(this.beanClass, ToolBytecode::isGetterOrSetterIgnoreCase);
        PropDesc prop;
        for (Field field : ToolBytecode.getFields(this.beanClass)) {
            // 排除静态属性和对象子类
            if (false == Whether.staticModifier(field) && !"this$0".equals(field.getName())) {
    ...
    private PropDesc findProp(Field field, Method[] gettersOrSetters, boolean ignoreCase) {
        ...
        final boolean isBooleanField = ToolBoolean.isBoolean(fieldType);
     ...
     * @return 是否匹配
     */
    private boolean isMatchGetter(
            String methodName, String fieldName, boolean isBooleanField, boolean ignoreCase) {
        ...
        if (ignoreCase) {
            ...
            handledFieldName = ToolString.upperFirst(fieldName);
        }
    ...
    private boolean isMatchSetter(
        ...
        if (isBooleanField && fieldName.startsWith("is")) {
            // 字段是is开头
            if (("set" + ToolString.removePrefix(fieldName, "is")).equals(methodName) // isName -》 setName
