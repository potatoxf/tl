...
public class BeanPath implements Serializable {
    ...
    private static final char[] EXP_CHARS = {
            PoolOfCharacter.DOT, PoolOfCharacter.BRACKET_START, PoolOfCharacter.BRACKET_END
    };
    ...
    private static boolean lastIsNumber(List<String> patternParts) {
        return ToolNumber.isInteger(patternParts.get(patternParts.size() - 1));
    }
    ...
    private static Object getFieldValue(final Object bean, final String expression) {
        if (Whether.blank(expression)) {
            return null;
        }

        if (ToolString.contains(expression, ':')) {
            // [start:end:step] 模式
            final List<String> parts = ToolString.splitTrim(expression, ':');
            ...
            if (bean instanceof Collection) {
                return ToolCollection.sub((Collection<?>) bean, start, end, step);
            } else if (Whether.arrayObj(bean)) {
                return ToolArray.sub(bean, start, end, step);
            }
        } else if (ToolString.contains(expression, ',')) {
            // [num0,num1,num2...]模式或者['key0','key1']模式
            final List<String> keys = ToolString.splitTrim(expression, ',');
            if (bean instanceof Collection) {
                return ToolCollection.getAny((Collection<?>) bean, Convert.convert(int[].class, keys));
            } else if (Whether.arrayObj(bean)) {
                return ToolArray.getAny(bean, Convert.convert(int[].class, keys));
            } else {
                final String[] unWrappedKeys = new String[keys.size()];
                for (int i = 0; i < unWrappedKeys.length; i++) {
                    unWrappedKeys[i] = ToolString.unWrap(keys.get(i), '\'');
                }
    ...
    private void set(Object bean, List<String> patternParts, boolean nextNumberPart, Object value) {
        ...
        if (null == subBean) {
            ...
            this.set(
                    bean,
                    parentParts,
                    lastIsNumber(parentParts),
                    nextNumberPart ? new ArrayList<>() : new HashMap<>());
    ...
    private void init(final String expression) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (false == isInWrap && ToolArray.contains(EXP_CHARS, c)) {
                // 处理边界符号
                if (PoolOfCharacter.BRACKET_END == c) {
                    // 中括号（数字下标）结束
                    if (false == isNumStart) {
                    ...
                    if (isNumStart) {
                        ...
                        throw new IllegalArgumentException(
                                ToolString.format(
                                        "Bad expression '{}':{}, we find '[' but no ']' !", expression, i));
                    } else if (PoolOfCharacter.BRACKET_START == c) {
                        // 数字下标开始
                        isNumStart = true;
                    }
