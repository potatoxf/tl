...
public class Arrangement implements Serializable {
     ...
     * 构造
     *
     * @param dataArray 用于排列的数据
     */
    public Arrangement(String[] dataArray) {
    ...
    public static long count(int n, int m) {
        if (n == m) {
            return ToolNumber.factorial(n);
        }
        return (n > m) ? ToolNumber.factorial(n, n - m) : 0;
    }
    ...
    public List<String[]> select() {
        return select(this.dataArray.length);
    }
    ...
    public List<String[]> select(int m) {
        ...
        select(this.dataArray, new String[m], 0, result);
        return result;
    }
    ...
    private void select(String[] datas, String[] resultList, int resultIndex, List<String[]> result) {
        ...
        for (int i = 0; i < datas.length; i++) {
            ...
            select(ToolArray.remove(datas, i), resultList, resultIndex + 1, result);
        }
    }
}
