...
public class Calculator {
    ...
    private final int[] operatePriority =
            new int[]{0, 3, 2, 1, -1, 1, 0, 2}; // 运用运算符ASCII码-40做索引的运算符优先级
    ...
    private static String transform(String expression) {
        expression = ToolString.cleanBlank(expression);
        expression = ToolString.removeSuffix(expression, "=");
    ...
    public double calculate(String expression) {
        ...
        while (false == Whether.empty(postfixStack)) {
    ...
    private boolean compare(char cur, char peek) { // 如果是peek优先级高于cur，返回true，默认都是peek优先级要低
        ...
        return operatePriority[peek - offset] >= operatePriority[cur - offset];
    }
    ...
    private BigDecimal calculate(String firstValue, String secondValue, char currentOp) {
        ...
        switch (currentOp) {
            case '+':
                result = ToolNumber.add(firstValue, secondValue);
                break;
            case '-':
                result = ToolNumber.sub(firstValue, secondValue);
                break;
            case '*':
                result = ToolNumber.mul(firstValue, secondValue);
                break;
            case '/':
                result = ToolNumber.div(firstValue, secondValue);
                break;
            case '%':
                result =
                        ToolNumber.toBigDecimal(firstValue).remainder(ToolNumber.toBigDecimal(secondValue));
                break;
            default:
