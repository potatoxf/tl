 ...
 *
 * <p>代码来自：https://github.com/rolandhe/string-tools<br>
...
public class CityHash {
    ...
    public static long hash64(byte[] data) {
        ...
        return hashLen16(
                hashLen16(v.getLowValue(), w.getLowValue()) + shiftMix(y) * k1 + z,
    ...
    public static Number128 hash128(byte[] data) {
        int len = data.length;
        return len >= 16
                ? hash128(data, 16, new Number128(fetch64(data, 0), fetch64(data, 8) + k0))
    ...
    private static Number128 hash128(final byte[] byteArray, int start, final Number128 seed) {
        ...
        do {
            ...
            w =
                    weakHashLen32WithSeeds(
                            byteArray, pos + 32, z + w.getHighValue(), y + fetch64(byteArray, pos + 16));
        ...
        for (int tail_done = 0; tail_done < len; ) {
            ...
            v =
                    weakHashLen32WithSeeds(
                            byteArray, pos + len - tail_done, v.getLowValue() + z, v.getHighValue());
            v.setLowValue(v.getLowValue() * k0);
        }
        ...
        return new Number128(
                hashLen16(x + v.getHighValue(), w.getHighValue()) + y,
    ...
    private static long fetch64(byte[] byteArray, int start) {
        return ToolByte.bytesToLong(byteArray, start, ToolByte.CPU_ENDIAN);
    }

    private static int fetch32(byte[] byteArray, final int start) {
        return ToolByte.bytesToInt(byteArray, start, ToolByte.CPU_ENDIAN);
    }
