package pxf.tl.lang.tree;


import pxf.tl.algs.tree.Tree;
import pxf.tl.iter.AnyIter;
import pxf.tl.lang.tree.parser.DefaultNodeParser;
import pxf.tl.lang.tree.parser.NodeParser;
...
public class TreeUtil {
    ...
    public static <E> Tree<E> buildSingle(Map<E, Tree<E>> map, E rootId) {
        final Tree<E> tree = AnyIter.ofIterable(map.values()).getFirst().orElse(null);
        if (null != tree) {
    ...
    public static <T> Tree<T> getNode(Tree<T> node, T id) {
        if (ToolObject.equal(id, node.getId())) {
            return node;
        }
