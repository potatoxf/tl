...
import pxf.tl.io.resource.FileObjectResource;
import pxf.tl.lang.ResourceClassLoader;
 ...
 * Java 字节码文件对象管理器
 *
 * <p>正常我们使用javac命令编译源码时会将class文件写入到磁盘中，但在运行时动态编译类不适合保存在磁盘中 我们采取此对象来管理运行时动态编译类生成的字节码。
 *
 * @author potatoxf
 */
class JavaClassFileManager extends ForwardingJavaFileManager<JavaFileManager> {
    ...
    protected JavaClassFileManager(ClassLoader parent, JavaFileManager fileManager) {
        super(fileManager);
        this.parent = Safe.value(parent, ToolBytecode::getClassLoader);
    }
