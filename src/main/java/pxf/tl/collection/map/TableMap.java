 ...
 * @author potatoxf
 */
public class TableMap<K, V> implements ExtendMap<K, V> {
    @Serial
    ...
    public TableMap(K[] keys, V[] values) {
        this.keys = New.list(false, keys);
        this.values = New.list(false, values);
    }
    ...
    public List<V> getValues(K key) {
        return ToolCollection.getAny(
                this.values, ToolCollection.indexOfAll(this.keys, (ele) -> ToolObject.equal(ele, key)));
    }
    ...
    public List<K> getKeys(V value) {
        return ToolCollection.getAny(
                this.keys, ToolCollection.indexOfAll(this.values, (ele) -> ToolObject.equal(ele, value)));
    }
    ...
    public Set<Entry<K, V>> entrySet() {
        ...
        for (int i = 0; i < size(); i++) {
            hashSet.add(Map.entry(keys.get(i), values.get(i)));
        }
        return hashSet;
    }
    ...
    public Iterator<Entry<K, V>> iterator() {
        return new Iterator<Entry<K, V>>() {
            ...
            public Entry<K, V> next() {
                return Map.entry(keysIter.next(), valuesIter.next());
            }
