package pxf.tl.collection.map.multi;

import pxf.tl.api.Builder;
import pxf.tl.collection.AbsEntry;
import pxf.tl.help.New;
import pxf.tl.iter.AbstractIter;
import pxf.tl.iter.TransIter;
...
public class RowKeyTable<R, C, V> extends AbsTable<R, C, V> {
    ...
    public RowKeyTable(boolean isLinked) {
        this(New.map(isLinked), () -> New.map(isLinked));
    }
    ...
    private class ColumnKeySet extends AbstractSet<C> {
        ...
        @Override
        public int size() {
            return ToolCollection.size(iterator());
        }
    }

    private class ColumnKeyIterator extends AbstractIter<C> {
    ...
    private class Column extends AbstractMap<R, V> {
        ...
        private class EntrySetIterator extends AbstractIter<Entry<R, V>> {
            final Iterator<Entry<R, Map<C, V>>> iterator = raw.entrySet().iterator();

            @Override
            protected Entry<R, V> doNext() {
