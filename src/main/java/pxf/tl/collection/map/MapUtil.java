...
public class MapUtil {
    ...
    public static <K, V> Map<K, V> createMap(Class<?> mapType) {
        if (null == mapType || mapType.isAssignableFrom(AbstractMap.class)) {
            ...
            try {
                return (Map<K, V>) ToolBytecode.createInstanceSilent(mapType);
    ...
    public static <K, V> HashMap<K, V> of(K key, V value, boolean isOrder) {
        final HashMap<K, V> map = New.map(isOrder);
        map.put(key, value);
        return map;
    }
    ...
    public static <K, V> Map<K, List<V>> toListMap(Iterable<? extends Map<K, V>> mapList) {
        ...
        for (Map<K, V> map : mapList) {
            ...
            for (Entry<K, V> entry : entrySet) {
                ...
                if (null == valueList) {
                    valueList = New.list(false, entry.getValue());
    ...
    public static <K, V> List<Map<K, V>> toMapList(Map<K, ? extends Iterable<V>> listMap) {
        ...
        do {
            ...
            for (Entry<K, ? extends Iterable<V>> entry : listMap.entrySet()) {
                vList = New.list(false, entry.getValue());
    ...
    public static <K, V> String join(
            Map<K, V> map, String separator, String keyValueSeparator, String... otherParams) {
    ...
    public static <K, V> String join(
            ...
            boolean isIgnoreNull,
            String... otherParams) {
        final StringBuilder strBuilder = ToolString.builder();
        ...
        if (Whether.noEmpty(map)) {
            for (Entry<K, V> entry : map.entrySet()) {
                if (!isIgnoreNull || entry.getKey() != null && entry.getValue() != null) {
                    ...
                    strBuilder
                            .append(Convert.toStr(entry.getKey()))
        ...
        // 补充其它字符串到末尾，默认无分隔符
        if (Whether.noEmpty(otherParams)) {
    ...
    public static <K, V> Map<K, V> edit(Map<K, V> map, Editor<Entry<K, V>> editor) {
        ...
        Map<K, V> map2 = ToolBytecode.createInstanceIfPossible(map.getClass());
        if (null == map2) {
    ...
    public static <K, V, R> Map<K, R> map(Map<K, V> map, BiFunction<K, V, R> biFunction) {
        ...
        return map.entrySet().stream()
                .collect(
                        ToolStream.toMap(
                                Entry::getKey, m -> biFunction.apply(m.getKey(), m.getValue()), (l, r) -> l));
    }
     ...
     * @param map Map
     * @return TreeMap
     * @see New#treeMap(Map, Comparator)
     */
    ...
    public static <K, V> TreeMap<K, V> sort(Map<K, V> map, Comparator<? super K> comparator) {
        ...
        return New.treeMap(map, comparator);
    }
    ...
    public static <K, V> Map<K, V> getAny(Map<K, V> map, final K... keys) {
        return filter(map, entry -> ToolArray.contains(keys, entry.getKey()));
    }
    ...
    public static <K, V> Map<K, V> renameKey(Map<K, V> map, K oldKey, K newKey) {
        if (Whether.empty(map) && map.containsKey(oldKey)) {
            if (map.containsKey(newKey)) {
                throw new IllegalArgumentException(ToolString.format("The key '{}' exist !", newKey));
            }
    ...
    public static <K, V, T extends Map<K, V>> T empty(Class<?> mapClass) {
        ...
        // 不支持空集合的集合类型
        throw new IllegalArgumentException(
                ToolString.format("[{}] is not support to get empty!", mapClass));
    }
