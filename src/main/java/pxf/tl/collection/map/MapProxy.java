...
public class MapProxy
        implements ExtendMap<Object, Object>,
        GetterWithKeyedForValue<Object>,
        InvocationHandler,
        Serializable {
    ...
    public Object invoke(Object proxy, Method method, Object[] args) {
        ...
        if (Whether.empty(parameterTypes)) {
            ...
            if (void.class != returnType) {
                ...
                if (methodName.startsWith("get")) {
                    // 匹配getXXX
                    fieldName = ToolString.removePreAndLowerFirst(methodName, 3);
                } else if (ToolBoolean.isBoolean(returnType) && methodName.startsWith("is")) {
                    // 匹配isXXX
                    fieldName = ToolString.removePreAndLowerFirst(methodName, 2);
                ...
                if (Whether.noBlank(fieldName)) {
                    if (false == this.containsKey(fieldName)) {
                        // 驼峰不存在转下划线尝试
                        fieldName = Safe.toUnderlineCase(fieldName,false);
                    }
            ...
            if (methodName.startsWith("set")) {
                final String fieldName = ToolString.removePreAndLowerFirst(methodName, 3);
                if (Whether.noBlank(fieldName)) {
    ...
    public <T> T toProxyBean(Class<T> interfaceClass) {
        return (T)
                Proxy.newProxyInstance(
                        ToolBytecode.getClassLoader(), new Class<?>[]{interfaceClass}, this);
    }
}
