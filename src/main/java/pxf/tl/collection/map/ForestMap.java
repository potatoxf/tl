 ...
 * @see TreeEntry
 */
public interface ForestMap<K, V> extends ExtendMap<K, TreeEntry<K, V>> {
    ...
    default void putAll(Map<? extends K, ? extends TreeEntry<K, V>> treeEntryMap) {
        if (Whether.empty(treeEntryMap)) {
            return;
        }
        treeEntryMap.forEach(
                (k, v) -> {
                    if (v.hasParent()) {
    ...
    default <C extends Collection<V>> void putAllNode(
        ...
        values.forEach(
                v -> {
                    ...
                    // 不忽略keu为null节点
                    final boolean hasKey = Whether.noNvl(key);
                    final boolean hasParentKey = Whether.noNvl(parentKey);
    ...
    default Set<TreeEntry<K, V>> getTreeNodes(K key) {
        ...
        final Set<TreeEntry<K, V>> results = New.set(true,target.getRoot());
        ToolCollection.addAll(results, target.getRoot().getChildren().values());
        return results;
    }
