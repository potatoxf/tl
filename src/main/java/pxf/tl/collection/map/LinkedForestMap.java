 ...
 *
 * <p>该集合可以被视为以{@link TreeEntryNode#getKey()}作为key，{@link TreeEntryNode}实例作为value的{@link
 * LinkedHashMap}。<br>
 * 使用时，将每一对键与值对视为一个{@link TreeEntryNode}节点，节点的id即为{@link TreeEntryNode#getKey()}，
 * 任何情况下使用相同的key都将会访问到同一个节点。<br>
 *
 * <p>节点通过key形成父子关系，并最终构成多叉树结构，多组平行的多叉树将在当前集合中构成森林。 使用者可以通过{@link ForestMap}本身的方法来对森林进行操作或访问，
...
public class LinkedForestMap<K, V> implements ForestMap<K, V> {
     ...
     *
     * <ul>
     *   <li>若存在父节点或子节点，则将其断开其与父节点或子节点的引用关系；
     *   <li>若同时存在父节点或子节点，则会在删除后将让子节点直接成为父节点的子节点，比如：<br>
    ...
    /**
     * 由key与{@link TreeEntry}组成的键值对实体的{@link Set}集合。 注意，返回集合中{@link Entry#setValue(Object)}不支持调用。
     *
     * @return 集合
     */
    @Override
     ...
     *
     * <ul>
     *   <li>若key对应节点不存在，则以传入的键值创建一个新的节点；
     *   <li>若key对应节点存在，则将该节点的值替换为{@code node}指定的值；
     * </ul>
     *
     * @param key   节点的key
     ...
     *
     * <ul>
     *   <li>若{@code parentKey}或{@code childKey}对应的节点不存在，则会根据键值创建一个对应的节点；
     *   <li>若{@code parentKey}或{@code childKey}对应的节点存在，则会更新对应节点的值；
     * </ul>
     * <p>
     * 该操作等同于：
     *
    ...
    public void linkNodes(
            K parentKey, K childKey, BiConsumer<TreeEntry<K, V>, TreeEntry<K, V>> consumer) {
        consumer = Safe.value(consumer, (parent, child) -> {
        });
        final TreeEntryNode<K, V> parentNode =
                nodes.computeIfAbsent(parentKey, t -> new TreeEntryNode<>(null, t));
        TreeEntryNode<K, V> childNode = nodes.get(childKey);

        // 1.子节点不存在
        if (Whether.nvl(childNode)) {
        ...
        if (ToolObject.equals(parentNode, childNode.getDeclaredParent())) {
            consumer.accept(parentNode, childNode);
            return;
        }
    ...
    public static class TreeEntryNode<K, V> implements TreeEntry<K, V> {
        ...
        TreeEntryNode<K, V> traverseParentNodes(
                ...
                Consumer<TreeEntryNode<K, V>> consumer,
                Predicate<TreeEntryNode<K, V>> breakTraverse) {
            breakTraverse = Safe.value(breakTraverse, n -> false);
            TreeEntryNode<K, V> curr = includeCurrent ? this : this.parent;
            while (Whether.noNvl(curr)) {
        ...
        public boolean equalsKey(K key) {
            return ToolObject.equal(getKey(), key);
        }
        ...
        TreeEntryNode<K, V> traverseChildNodes(
                boolean includeCurrent,
                BiConsumer<Integer, TreeEntryNode<K, V>> consumer,
                BiPredicate<Integer, TreeEntryNode<K, V>> breakTraverse) {
            breakTraverse = Safe.value(breakTraverse, (i, n) -> false);
            final Deque<List<TreeEntryNode<K, V>>> keyNodeDeque = new LinkedList<>();
            keyNodeDeque.add(New.list(false, this.children.values()));
            ...
            while (!keyNodeDeque.isEmpty()) {
                ...
                for (final TreeEntryNode<K, V> node : curr) {
                    ...
                    ToolCollection.addAll(next, node.children.values());
                }
                if (!Whether.empty(next)) {
                    keyNodeDeque.addLast(next);
                }
                lastNode = ToolCollection.getLast(next);
                index++;
            }
            return lastNode;
        }
        ...
        void addChild(TreeEntryNode<K, V> child) {
            ...
            traverseParentNodes(
                    ...
                    s ->
                            Assert.notEquals(
                                    s.key, child.key, "circular reference between [{}] and [{}]!", s.key, this.key),
                    null);
        ...
        public boolean equals(Object o) {
            ...
            if (o == null
                    ...
                    || ToolClass.isAssignable(this.getClass(), o.getClass())) {
                return false;
            }
            final TreeEntry<?, ?> treeEntry = (TreeEntry<?, ?>) o;
            return ToolObject.equals(this.getKey(), treeEntry.getKey());
        }
        ...
        TreeEntryNode<K, V> copy(V value) {
            TreeEntryNode<K, V> copiedNode =
                    new TreeEntryNode<>(this.parent, this.key, Safe.value(value, this.value));
    ...
    public static class EntryNodeWrapper<K, V, N extends TreeEntry<K, V>>
            implements Entry<K, TreeEntry<K, V>> {
